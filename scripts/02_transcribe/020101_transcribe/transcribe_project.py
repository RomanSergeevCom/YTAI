#!/usr/bin/env python3
"""
YTAI Transcribe v3.0 (020101_transcribe)
...
Изменения от v2.16:
- Пути JSON/SRT → внутрь _transcription/ (рядом с видео только xlsx)
- Stage 5b (prproj) удалён → Premiere проект через UXP плагин
- Stage 6: генерация ingest JSON
- Флаг --multicam: мультикамера без мастер-файлов

Usage:
    python transcribe_project.py --project "/path/to/Interview" -n 2
    python transcribe_project.py --project "/path/to/video.MP4" -n 2 -y
    python transcribe_project.py --project "/path/to/MultiCam" -n 2 --multicam -y
"""
...
def print_stage_header(num, total, text, stage_key, est_times, st, str_set,
                       pipeline_start=None):
    """Extended stage header: shows plan estimate + total elapsed + remaining.

    Example output:
      Stage 3/6  Whisper transcription (word_timestamps)
      ╸ Plan ~11m 24s  ·  Elapsed 1m 20s  ·  Left ~11m 28s
    """
    drift = compute_drift(st, est_times)
    plan_raw = est_times.get(stage_key, 0)
    plan_adj = plan_raw * drift

    # Compute remaining: current stage plan + all stages after
    ...
    remaining = plan_adj + after_est

    # Line 1: stage header
    print(f"\n  {C.BG_CYAN}{C.WHITE}{C.BOLD} Stage {num}/{total} {C.RESET} {C.BOLD}{text}{C.RESET}")
    # Line 2: plan + elapsed + remaining
    plan_str = f"~{fmt_time(plan_adj)}" if plan_adj > 0 else "—"
    if pipeline_start is not None:
        elapsed_total = time.time() - pipeline_start
        remaining_str = f"~{fmt_time(remaining)}" if remaining > 0 else "—"
        print(f"  {C.DIM}╸ Plan {plan_str}  ·  Elapsed {fmt_time(elapsed_total)}  ·  Left {remaining_str}{C.RESET}")
    else:
        # Fallback (no pipeline_start, e.g. dry-run)
        print(f"  {C.DIM}╸ Plan {plan_str}{C.RESET}")

def print_done(text, elapsed):
    print(f"  {C.GREEN}{C.BOLD}✓{C.RESET} {text}  {C.DIM}{fmt_time(elapsed)}{C.RESET}")

def print_stage_done(ctx, text, elapsed, stage_key):
    """Print stage completion with plan comparison + pipeline timing.

    Example output:
      ✓ Stage 2 complete  1m 15s  (plan ~1m 40s, −25s)  ·  Elapsed 2m 35s  ·  Left ~11m 28s
    """
    pipeline_start = ctx.get("_pipeline_start")
    if pipeline_start is None:
        # Fallback to simple print_done
        print_done(text, elapsed)
        return

    # Plan vs actual delta
    plan_adj = ctx.get("_stage_plan_adj", 0)
    if plan_adj > 0:
        delta = elapsed - plan_adj
        d_sign = "+" if delta >= 0 else "−"
        delta_str = f"  (plan ~{fmt_time(plan_adj)}, {d_sign}{fmt_time(abs(delta))})"
    else:
        delta_str = ""
    ...
    # Remaining: sum of stages after current one
    est_times = ctx.get("stage_estimates", {})
    st = ctx.get("stages_timing", {})
    str_set = set(st.keys()) | {stage_key}  # completed + current
    drift = ctx.get("_current_drift", 1.0)
    stage_order = ["1", "2", "3", "4", "5", "6"]
    try:
        idx = stage_order.index(stage_key)
    except ValueError:
        idx = len(stage_order)
    # Remaining stages = those after current that are in the run set
    run_set = set(ctx.get("_run_stages", stage_order))
    after_keys = [k for k in stage_order[idx + 1:] if k in run_set]
    remaining = sum(est_times.get(k, 0) for k in after_keys) * drift
    remaining_str = f"~{fmt_time(remaining)}" if remaining > 0 else "—"

    print(f"  {C.GREEN}{C.BOLD}✓{C.RESET} {text}  {C.DIM}{fmt_time(elapsed)}{delta_str}"
          f"  ·  Elapsed {fmt_time(elapsed_total)}  ·  Left {remaining_str}{C.RESET}")
...
class PipelineProgress:
    ...
    STAGE_WEIGHTS = {
        "1": 0.003, "2": 0.06, "3": 0.93, "4": 0.001, "5": 0.003,
        "6": 0.001,
    }
...
def get_media_info(video_path):
    ...
    return {
        ...
        "audio_channel_layout": as_.get("channel_layout", ""),
        ...
        "sample_aspect_ratio": vs.get("sample_aspect_ratio", ""),
        "rotation": vs.get("tags", {}).get("rotate", ""),
    }


def parse_camera_xml(video_path):
    """Parse Sony NonRealTimeMeta XML sidecar if exists next to the video file.

    Looks for {clip_stem}M01.XML next to {clip_stem}.MP4.
    Returns dict with camera metadata or empty dict if no XML found.
    """
    import xml.etree.ElementTree as ET
    video_path = Path(video_path)
    xml_path = video_path.parent / f"{video_path.stem}M01.XML"
    ...
    try:
        ...
        # Device
        dev = root.find("nrt:Device", ns)
        if dev is not None:
            result["device_manufacturer"] = dev.get("manufacturer", "")
            result["device_model"] = dev.get("modelName", "")
            result["device_serial"] = dev.get("serialNo", "")
            result["device"] = f"{dev.get('manufacturer', '')} {dev.get('modelName', '')}".strip()
        # Lens
        lens = root.find("nrt:Lens", ns)
        if lens is not None:
            result["lens"] = lens.get("modelName", "")
        # Duration (in frames)
        dur_el = root.find("nrt:Duration", ns)
        if dur_el is not None:
            result["duration_frames"] = int(dur_el.get("value", 0))
        # CreationDate
        cd = root.find("nrt:CreationDate", ns)
        if cd is not None:
            result["creation_date"] = cd.get("value", "")
        # UMID
        tm = root.find("nrt:TargetMaterial", ns)
        if tm is not None:
            result["umid"] = tm.get("umidRef", "")
        # LTC Timecode
        ltc_table = root.find("nrt:LtcChangeTable", ns)
        if ltc_table is not None:
            result["ltc_fps"] = int(ltc_table.get("tcFps", 0))
            first_ltc = ltc_table.find("nrt:LtcChange", ns)
            if first_ltc is not None:
                tc_raw = first_ltc.get("value", "")
                if len(tc_raw) == 8:
                    result["ltc_start"] = f"{tc_raw[0:2]}:{tc_raw[2:4]}:{tc_raw[4:6]}:{tc_raw[6:8]}"
                else:
                    result["ltc_start"] = tc_raw
        # VideoFormat
        vf_el = root.find("nrt:VideoFormat/nrt:VideoFrame", ns)
        if vf_el is not None:
            result["video_codec_full"] = vf_el.get("videoCodec", "")
            result["capture_fps"] = vf_el.get("captureFps", "")
        vl_el = root.find("nrt:VideoFormat/nrt:VideoLayout", ns)
        if vl_el is not None:
            result["aspect_ratio"] = vl_el.get("aspectRatio", "")
        # AudioFormat
        af = root.find("nrt:AudioFormat", ns)
        if af is not None:
            result["audio_channels_xml"] = int(af.get("numOfChannel", 0))
            ports = af.findall("nrt:AudioRecPort", ns)
            if ports:
                result["audio_codec_xml"] = ports[0].get("audioCodec", "")
        # AcquisitionRecord — gamma, color primaries, coding equations
        for item in root.findall("nrt:AcquisitionRecord/nrt:Group/nrt:Item", ns):
            name = item.get("name", "")
            value = item.get("value", "")
            if name == "CaptureGammaEquation":
                result["gamma"] = value
            elif name == "CaptureColorPrimaries":
                result["color_primaries"] = value
            elif name == "CodingEquations":
                result["coding_equations"] = value
        # RelevantFiles — LUT
        for rel in root.findall("nrt:RelevantFiles/nrt:RelatedTo", ns):
            if rel.get("rel") == "LUT":
                result["lut"] = rel.get("file", "")
        # RecordingMode
        rm = root.find("nrt:RecordingMode", ns)
        if rm is not None:
            result["recording_mode"] = rm.get("type", "")
        return result
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="YTAI Transcribe v3.0 — Word-level transcription with speaker diarization + ingest JSON"
    )
...
def preflight(ctx, args):
    ...
    if args.dry_run:
        ...
        total_duration = 0.0; clip_durations = {}; clip_media_info = {}; clip_camera_meta = {}
        for vf in ctx["video_files"]:
            ...
            cam_meta = parse_camera_xml(vf)
            if cam_meta:
                clip_camera_meta[vf.stem] = cam_meta
        ctx["clip_durations"] = clip_durations
        ctx["clip_media_info"] = clip_media_info
        ctx["clip_camera_meta"] = clip_camera_meta
        ...
        ctx["stage_estimates"] = {
            ...
            "6": td * SPEED_COEFFICIENTS["ingest_json"],
        }
        ...
        if clip_camera_meta:
            first_cam = next(iter(clip_camera_meta.values()))
            cam_str = first_cam.get("device", "?")
            lens_str = first_cam.get("lens", "")
            gamma_str = first_cam.get("gamma", "")
            parts = [cam_str]
            if lens_str:
                parts.append(lens_str)
            if gamma_str:
                parts.append(gamma_str)
            print(f"  {C.GREEN}✓{C.RESET} Camera XML: {len(clip_camera_meta)} sidecar{'s' if len(clip_camera_meta) > 1 else ''} — {' | '.join(parts)}")
        return {}
    ...
    # 7. Video files — duration + media metadata + camera XML sidecars
    total_duration = 0.0
    clip_durations = {}
    clip_media_info = {}
    clip_camera_meta = {}
    ...
    stage_estimates = {
        ...
        "6": total_duration * SPEED_COEFFICIENTS["ingest_json"],
    }
...
def stage1_extract_audio(ctx, args):
    ...
    for i, vf in enumerate(clips):
        ...
        # Move camera XML sidecar to per_clip dir if exists
        xml_sidecar = vf.parent / f"{clip_id}M01.XML"
        if xml_sidecar.exists():
            dest_xml = clip_dir / xml_sidecar.name
            if not dest_xml.exists():
                shutil.move(str(xml_sidecar), str(dest_xml))
                # Update xml_path in camera_meta
                cam = ctx.get("clip_camera_meta", {}).get(clip_id, {})
                if cam:
                    cam["xml_path"] = str(dest_xml)
    ...
    print_stage_done(ctx, f"Stage 1 complete — {len(clips)} clips, {fmt_duration(total_duration)}", elapsed, "1")
    return elapsed
...
def stage2_diarization(ctx, args, resources):
    ...
    elapsed = time.time() - t0
    print_stage_done(ctx, "Stage 2 complete", elapsed, "2")
    return elapsed
...
def stage3_whisper(ctx, args, resources):
    ...
    for i, vf in enumerate(clips):
        ...
        # Store per-clip timing for logs
        ctx.setdefault("whisper_clip_timing", {})[clip_id] = {
            "elapsed": round(elapsed_clip, 1),
            "speed": round(speed, 2),
            "words": clip_words,
            "language": lang,
        }
    ...
    elapsed = time.time() - t0
    print_stage_done(ctx, "Stage 3 complete", elapsed, "3")
    return elapsed
...
def stage4_speaker_mapping(ctx, args):
    ...
    elapsed = time.time() - t0
    print_stage_done(ctx, "Stage 4 complete", elapsed, "4")
    return elapsed
...
def split_word_punctuation(whisper_word):
    """Convert whisper word to Adobe Transcript JSON format (seconds, not ms)."""
    text = whisper_word["word"].strip()
    s = round(whisper_word["start"], 3)
    dur = round(whisper_word["end"] - whisper_word["start"], 3)
...
def generate_premiere_json(clip_transcript, speaker_map, language):
    ...
    for seg in clip_transcript["segments"]:
        ...
        seg_start = prem_words[0]["start"] if prem_words else 0
        last_w = prem_words[-1] if prem_words else None
        seg_dur = round((last_w["start"] + last_w["duration"] - seg_start), 3) if last_w else 0
        prem_segments.append({
            "language": prem_lang,
            "speaker": sid,
            "start": seg_start,
            "duration": seg_dur,
...
def generate_word_level_srt(clip_transcript, speaker_map, words_per_line=4):
    """Generate word-level SRT with 2-line caption blocks.

    Each block has 2 lines of ~4 words each (8 words total).
    Speaker tag shown only on speaker change (multi-speaker only).
    """
    lines = []
    idx = 1
    multi_speaker = len(speaker_map) > 1
    prev_speaker = None
    ...
    for seg in clip_transcript["segments"]:
        ...
        if not words:
            continue
        speaker = seg["speaker"]
        name = speaker_map.get(speaker, {}).get("name", speaker)
        words_per_block = words_per_line * 2
        ...
        for i in range(0, len(words), words_per_block):
            ...
            start = block[0]["start"]
            last = block[-1]
            end = last["end"] if "end" in last else last["start"] + last.get("duration", 0.5)
            if end - start < 0.3:
                end = start + 0.3

            line1_words = block[:words_per_line]
            line2_words = block[words_per_line:]
            line1 = " ".join(w.get("word", w.get("text", "")) for w in line1_words)
            line2 = " ".join(w.get("word", w.get("text", "")) for w in line2_words)

            prefix = ""
            if multi_speaker and speaker != prev_speaker:
                prefix = f"[{name}] "
                prev_speaker = speaker

            text = f"{prefix}{line1}\n{line2}" if line2 else f"{prefix}{line1}"
            if not text.strip():
                continue
...
def generate_combined_word_level_srt(ctx, clip_filter=None, output_path=None, words_per_line=4):
    """Generate word-level combined SRT with 2-line caption blocks.

    Each block has 2 lines of ~4 words. Speaker tag only on speaker change.
    """
    smap = ctx["speaker_map"]
    offsets = ctx["clip_offsets_flat"]
    video_files = ctx["video_files"]
    ...
    for vf in video_files:
        ...
        for seg in tr["segments"]:
            ...
            if not words:
                continue
            speaker = seg["speaker"]
            name = smap.get(speaker, {}).get("name", speaker)
            for i in range(0, len(words), words_per_block):
                block = words[i:i + words_per_block]
                if not block:
                    continue
                start = co + block[0]["start"]
                last = block[-1]
                end = co + (last["end"] if "end" in last else last["start"] + last.get("duration", 0.5))
                if end - start < 0.3:
                    end = start + 0.3
                line1_words = block[:words_per_line]
                line2_words = block[words_per_line:]
                line1 = " ".join(w["word"] for w in line1_words)
                line2 = " ".join(w["word"] for w in line2_words)
                prefix = ""
                if multi_speaker and speaker != prev_speaker:
                    prefix = f"[{name}] "
                    prev_speaker = speaker
...
def generate_xlsx(ctx, args, clip_filter=None, output_path=None):
    ...
    media_headers = ["Clip", "File", "Duration", "Resolution", "FPS", "Frames",
                     ...
                     "Camera", "Lens", "Gamma", "Color Space", "LTC Start", "LUT"]
    ...
    clip_cam = ctx.get("clip_camera_meta", {})
    mr = 2
    total_frames = 0
    total_size = 0
    for vf in video_files:
        ...
        # Camera XML columns
        cam = clip_cam.get(vf.stem, {})
        ws3.cell(row=mr, column=14, value=cam.get("device", ""))
        ws3.cell(row=mr, column=15, value=cam.get("lens", ""))
        ws3.cell(row=mr, column=16, value=cam.get("gamma", ""))
        ws3.cell(row=mr, column=17, value=cam.get("color_primaries", ""))
        ws3.cell(row=mr, column=18, value=cam.get("ltc_start", ""))
        ws3.cell(row=mr, column=19, value=cam.get("lut", ""))
...
def generate_project_json(ctx, args, clip_filter=None, output_path=None):
    ...
    camera_meta = ctx.get("clip_camera_meta", {})
    ...
    for vf in video_files:
        ...
        # Camera XML metadata (Sony NonRealTimeMeta sidecar)
        cam = camera_meta.get(cid)
        if cam:
            cam_out = {}
            for key in ("device", "device_manufacturer", "device_model", "device_serial",
                        "lens", "gamma", "color_primaries", "coding_equations", "lut",
                        "ltc_start", "ltc_fps", "creation_date", "umid",
                        "duration_frames", "video_codec_full", "capture_fps",
                        "aspect_ratio", "audio_codec_xml", "audio_channels_xml",
                        "recording_mode"):
                val = cam.get(key)
                if val:
                    cam_out[key] = val
            # Store relative XML path
            xml_abs = cam.get("xml_path", "")
            if xml_abs:
                try:
                    cam_out["xml_source"] = str(Path(xml_abs).relative_to(project_json_dir))
                except ValueError:
                    cam_out["xml_source"] = xml_abs
            clip_entry["camera_meta"] = cam_out
    ...
    if groups and not clip_filter:
        ...
        for grp_name, grp in groups.items():
            cameras[grp_name] = {
                ...
                "transcript_json": f"{grp_name}/{grp_name}_transcription/{grp_name}_transcript.json",
                "srt": f"{grp_name}/{grp_name}_transcription/{grp_name}_transcript.srt",
            }
        structure = {
            "type": "multi-camera",
            ...
            "combined": {
                ...
                "transcript_json": f"{ctx['project_name']}_transcription/{ctx['project_name']}_transcript.json",
                "srt": f"{ctx['project_name']}_transcription/{ctx['project_name']}_transcript.srt",
            },
        }
...
def stage5_generate_outputs(ctx, args):
    ...
    for idx, vf in enumerate(clips):
        ...
        with open(cd/f"{cid}_captions.srt","w") as f: f.write(generate_word_level_srt(tr,smap))
    ...
    if groups:
        for grp_name, grp in groups.items():
            grp_td = grp["source_dir"] / f"{grp_name}_transcription"
            grp_td.mkdir(parents=True, exist_ok=True)
            ...
            grp_proj = generate_project_json(ctx, args, clip_filter=grp["clip_ids"],
                output_path=grp_td / f"{grp_name}_transcript.json")
            print(f"  {grp_name}/{grp_name}_transcription/{grp_proj.name}")
...
def stage6_generate_ingest_json(ctx, args):
    """Stage 6: Generate ingest JSON + copy Premiere template."""
    t0 = time.time()
...
def save_meta(ctx, args, resources, st):
    td = ctx["transcription_dir"]
    comp = [k for k in ["1","2","3","4","5","6"] if k in st]
    ...
    meta = {"version": VERSION, "status": status,
        ...
        "stages_timing": {"1_extract_audio": round(st.get("1",0),1),
            ...
            "6_ingest_json": round(st.get("6",0),1)},
        "total_time": round(sum(st.values()),1)}
    # Camera metadata from XML sidecars (if found)
    cam_meta = ctx.get("clip_camera_meta", {})
    if cam_meta:
        first_cam = next(iter(cam_meta.values()))
        meta["camera_meta"] = {
            "device": first_cam.get("device", ""),
            "lens": first_cam.get("lens", ""),
            "gamma": first_cam.get("gamma", ""),
            "color_primaries": first_cam.get("color_primaries", ""),
            "coding_equations": first_cam.get("coding_equations", ""),
...
def write_log(ctx, args, resources, st):
    ...
    lines = [
        ...
        f"Multicam:   {'yes' if args.multicam else 'no'}",
        "",
        "=== Clips ===",
    ]
    clip_media = ctx.get("clip_media_info", {})
    for vf in ctx["video_files"]:
        ...
        fps = info.get("fps", "")
        vc = info.get("video_codec", "").upper()
        bitrate_mbps = round(int(info.get("video_bitrate", 0)) / 1_000_000, 1) if info.get("video_bitrate") else 0
        pix_fmt = info.get("pix_fmt", "")
        media_str = f"  {res} {fps}fps {vc}" if w else ""
        if bitrate_mbps:
            media_str += f"  {bitrate_mbps}Mbps"
        if pix_fmt:
            media_str += f" {pix_fmt}"
        lines.append(f"  {cid:<20s}  {fmt_duration(dur):>6s}{media_str}")
        # Extended line: color, audio, file size
        color_parts = []
        for ck in ("color_primaries", "color_transfer", "color_space"):
            cv = info.get(ck, "")
            if cv:
                color_parts.append(cv)
        ac = info.get("audio_codec", "")
        asr = info.get("audio_sample_rate", "")
        ach = info.get("audio_channel_layout", "") or (f"{info.get('audio_channels', '')}ch" if info.get("audio_channels") else "")
        fsize = info.get("file_size", 0)
        fsize_gb = round(fsize / (1024**3), 2) if fsize else 0
        ext_parts = []
        if color_parts:
            ext_parts.append(f"Color: {'/'.join(color_parts)}")
        if ac:
            ext_parts.append(f"Audio: {ac} {asr}Hz {ach}")
        if fsize_gb:
            ext_parts.append(f"Size: {fsize_gb} GB")
        if ext_parts:
            lines.append(f"    {'  '.join(ext_parts)}")
        lines.append(f"    offset {offset:.3f}s  {vf.name}")

    # Camera metadata from XML sidecars
    clip_cam = ctx.get("clip_camera_meta", {})
    if clip_cam:
        first_cam = next(iter(clip_cam.values()))
        lines += [
            "",
            "=== Camera ===",
            f"Device:     {first_cam.get('device', '?')}",
        ]
        if first_cam.get("lens"):
            lines.append(f"Lens:       {first_cam['lens']}")
        if first_cam.get("gamma"):
            lines.append(f"Gamma:      {first_cam['gamma']}")
        if first_cam.get("color_primaries"):
            lines.append(f"Color:      {first_cam['color_primaries']}")
        if first_cam.get("coding_equations"):
            lines.append(f"Encoding:   {first_cam['coding_equations']}")
        if first_cam.get("lut"):
            lines.append(f"LUT:        {first_cam['lut']}")
        lines.append(f"Sidecars:   {len(clip_cam)}")
    ...
    # Whisper per-clip timing
    wct = ctx.get("whisper_clip_timing", {})
    if wct:
        lines.append("=== Whisper Per-Clip ===")
        for vf in ctx["video_files"]:
            cid = vf.stem
            wt = wct.get(cid)
            if not wt:
                continue
            dur = ctx.get("clip_durations", {}).get(cid, 0)
            words = wt.get("words", 0)
            wpm = round(words / (dur / 60)) if dur > 0 else 0
            lines.append(f"  {cid:<20s}  {fmt_duration(dur):>6s}  {words:>5} words  {wpm:>4} wpm  "
                         f"{fmt_time(wt['elapsed']):>8s}  ({wt['speed']:.2f}x)  {wt.get('language', '?')}")
        lines.append("")
    ...
    # Processing speed and wall clock
    speed_x = ctx["total_duration"] / total if total > 0 else 0
    lines.append(f"  Speed:          {speed_x:.1f}x realtime")
    pipeline_start = ctx.get("_pipeline_start")
    if pipeline_start:
        wall_clock = time.time() - pipeline_start
        lines.append(f"  Wall clock:     {fmt_time(wall_clock)}")
    ...
    try:
        import whisper
        lines.append(f"Whisper:    {whisper.__version__ if hasattr(whisper, '__version__') else '?'}")
    except ImportError:
        pass
    try:
        import pyannote.audio
        lines.append(f"Pyannote:   {pyannote.audio.__version__ if hasattr(pyannote.audio, '__version__') else '?'}")
    except (ImportError, AttributeError):
        pass
    lines.append(f"Model:      {args.model}")
    lines.append(f"Device:     {resources['device']}")
    ...
    for s in ["1","2","3","4","5","6"]:
        if s not in completed:
            ...
            stage_key_map = {"1_extract_audio": "1", "2_diarization": "2",
                             ...
                             "5_generate_outputs": "5", "6_ingest_json": "6"}
...
def main():
    ...
    if args.dry_run:
        str_set = set(args.stages.split(",")) if args.stages else {"1","2","3","4","5","6"}
    ...
    ctx["multicam"] = args.multicam
    pipeline_prog = PipelineProgress()
    ctx["_pipeline_start"] = pipeline_prog.pipeline_start
    ctx["_run_stages"] = str_set
    ...
    for s in ["1","2","3","4","5","6"]:
        ...
        # Stage dispatch table: label, function, extra args
        stage_dispatch = {
            "1": ("Extracting audio",
                  lambda: stage1_extract_audio(ctx, args)),
            "2": (f"Speaker diarization ({fmt_duration(ctx['total_duration'])})",
                  lambda: stage2_diarization(ctx, args, resources)),
            "3": ("Whisper transcription (word_timestamps)",
                  lambda: stage3_whisper(ctx, args, resources)),
            "4": ("Mapping speakers",
                  lambda: stage4_speaker_mapping(ctx, args)),
            "5": ("Generating outputs",
                  lambda: stage5_generate_outputs(ctx, args)),
            "6": ("Generating ingest JSON",
                  lambda: stage6_generate_ingest_json(ctx, args)),
        }
        label, run_fn = stage_dispatch[s]
        print_stage_header(s, "6", label, s, est_times, st, str_set,
                           pipeline_start=pipeline_prog.pipeline_start)
        st[s] = run_fn()
