# transcribe_project v2.11 — Спецификация
...
**Скрипт:** ~/YTAI/scripts/02_transcribe/transcribe_project_v2.11.py
**Версия:** 2.11
...
## Окружение
    ...
    python ~/YTAI/scripts/02_transcribe/transcribe_project_v2.11.py \
...
### Вариант A: Папка с видео
    ...
    python transcribe_project_v2.11.py --project "/path/to/Interview" -n 2
...
### Вариант B: Один видеофайл
    ...
    python transcribe_project_v2.11.py --project "/path/to/C5090.MP4" -n 2
...
### Параметры запуска
    ...
    --lut PATH          конкретный LUT файл
    --lut-name NAME     dark/normal/bright (без анализа)
    --no-lut            не копировать LUT
...
- **`--stages`** — должен быть подмножеством `{1, 2, 3, 4, 5, 5b, 5c}`. При невалидном значении: `ERROR: invalid stage(s): {invalid}. Valid stages: 1, 2, 3, 4, 5, 5b, 5c`
...
### Проверки
    ...
    11. LUT files           ~/YTAI/luts/*.cube (WARN if missing)
...
## Pipeline
    ...
    Stage 5: Generate Outputs
      xlsx, SRT, TXT, internal JSON, Premiere JSON, brief.json, meta.json

    Stage 5b: Generate Premiere Project
      Template .prproj -> inject clips on V1/A1 timeline

    Stage 5c: LUT Selection
      Analyze first clip exposure (S-Log3) -> copy dark/normal/bright.cube
...
> - Brief: {project_name}_brief.json (рядом с XLSX, на уровне проекта)
...
### 5.2 meta.json

    {
      ...
      "stages_completed": ["1", "2", "3", "4", "5", "5b", "5c"],
      "stages_timing": {
        "1_extract_audio": 135.2, "2_diarization": 588.4,
        "3_whisper": 1112.8, "4_speaker_mapping": 4.1, "5_generate_outputs": 7.9,
        "5b_premiere_project": 2.1, "5c_lut_selection": 3.4
      },
      "lut": {"name": "normal", "brightness": 0.42},
      "total_time": 1853.9
    }
...
Рядом с видеофайлами (не в _transcription/). **4 листа: Transcript, Summary, Brief, Media.**
...
#### Sheet 3: "Brief" — для подготовки брифа (НОВЫЙ)

Упрощённый вид для пометок — какие части использовать в монтаже.

    Clip  | Timecode | Duration | Words | Text                                    | Use | Notes
    C5402 | 00:00:01 | 2:30     | 328   | The other day my colleague asked in...  |     |
    C5403 | 00:02:36 | 1:11     | 167   | Long story short. Yeah, long story...   |     |
    C5404 | 00:05:54 | 0:01     | 6     | That's what you're talking about.       |     | ⚠ low confidence

- **Use** — пустая колонка, пользователь заполняет вручную
- **Notes** — авто-заполняется "⚠ low confidence" для сегментов с confidence < 0.7
- Clip separators: те же что на Sheet 1 (light blue)
- Text: wrap_text, width=80

#### Sheet 4: "Media" — техническая информация по клипам
...
## Stage 5c: LUT Selection and Placement

### Обзор

Скрипт определяет экспозицию съёмки и выбирает подходящий LUT из трёх вариантов. Все клипы одного проекта получают один LUT (одна съёмка = одна экспозиция).

### LUT файлы

    ~/YTAI/luts/
      dark.cube       # для недосвета (S-Log3 avg brightness < 35%)
      normal.cube     # для нормальной экспозиции (35-55%)
      bright.cube     # для пересвета (> 55%)

### Алгоритм определения экспозиции

    1. Взять первый клип проекта
    2. Извлечь 10 кадров равномерно по длительности
    3. Конвертировать в grayscale
    4. Посчитать median яркости (0.0 - 1.0)
    5. Сравнить с порогами
...
Код:
    ...
    def analyze_exposure(video_path, num_frames=10):
        """Analyze S-Log3 footage exposure from sampled frames."""
        duration = get_duration(video_path)  # ffprobe
        timestamps = np.linspace(1, duration - 1, num_frames)
        brightnesses = []

        for ts in timestamps:
            # Extract single gray frame
            cmd = [
                "ffmpeg", "-ss", str(ts), "-i", str(video_path),
                "-frames:v", "1", "-f", "rawvideo",
                "-pix_fmt", "gray", "-s", "320x180",  # downscale
                "-v", "quiet", "pipe:1"
            ]
    ...
    def select_lut(brightness):
        """Select LUT based on S-Log3 brightness levels."""
        # S-Log3 пороги (калибровать по реальным съёмкам)
        if brightness < 0.35:
            return "dark"
...
Выполняется как подэтап Stage 5 (Generate Outputs), после основных файлов:

    1. analyze_exposure(first_clip) -> brightness
    2. select_lut(brightness) -> lut_name
    3. Скопировать ~/YTAI/luts/{lut_name}.cube -> {transcription_dir}/lut/
    4. Записать выбор в meta.json: "lut": {"name": "normal", "brightness": 0.42}
...
### Размещение LUT
    ...
    +-- Interview_transcription/
        +-- lut/
            +-- normal.cube       (выбранный LUT)
            +-- lut_info.json     (brightness, выбор, пороги)

Также копируется в work_dir рядом с .prproj для удобства:

    Interview/
    +-- Interview.prproj
    +-- normal.cube               (копия для быстрого доступа из Premiere)

lut_info.json:

    {
      "selected": "normal",
      "median_brightness": 0.42,
      "thresholds": {"dark": 0.35, "bright": 0.55},
      "source_clip": "C5090.MP4",
      "frames_analyzed": 10,
      "all_luts": ["dark.cube", "normal.cube", "bright.cube"]
    }

### Применение в .prproj

LUT НЕ встраивается в .prproj автоматически (слишком хрупко — Lumetri effect structure varies by version). Вместо этого:
- LUT файл копируется в папку проекта
- lut_info.json содержит рекомендацию
- В терминале показываем: "Recommended LUT: normal (brightness: 0.42)"
- Пользователь применяет в Premiere: Lumetri Color -> Basic -> Input LUT -> Browse

### Будущее (v2.12+)

- Встраивание LUT в .prproj через Lumetri effect в VideoComponentChain
- Per-clip LUT для проектов с разным освещением
- Калибровка порогов через --calibrate (снять серую карту, записать brightness)

### Параметры

    --lut PATH          путь к конкретному LUT (пропускает автоопределение)
    --lut-name NAME     dark/normal/bright (пропускает анализ)
    --no-lut            не копировать LUT
...
### clip_offsets.json
    ...
    {
      "version": "2.11", "created_at": "2026-01-13T12:22:00Z",
      "clips": [
...
### diarization.json
    ...
    {
      "version": "2.11", "created_at": "2026-01-13T12:30:00Z",
...
**Слова НЕ включены** — доступны в per-clip JSON.
    ...
    {
      "version": "2.11", "created_at": "2026-01-13T12:50:47Z", "project": "Interview",
...
### brief.json ({project_name}_brief.json)

Генерируется в Stage 5 рядом с XLSX (на уровне проекта, не в _transcription/). Предназначен для UXP-плагина Premiere — содержит всю информацию для просмотра, пометок и отбора сегментов.
      ...
      "clips": [
        {
          ...
          "filename": "C5402.MP4",
          "duration": 156.0,
          "premiere_transcript": "Interview_transcription/per_clip/C5402/C5402_premiere_transcript.json",
          "segments": [
            {
              "id": 0,
              "timecode": "00:00:01",
...
- `project` — имя проекта (project_name)
- `created_at` — ISO timestamp генерации
- `language` — определённый язык
- `speakers` — маппинг спикеров (из speakers.json)
- `clips[]` — массив клипов:
  - `clip_id` — stem имени файла
  - `filename` — полное имя файла
  - `duration` — длительность в секундах
  - `premiere_transcript` — относительный путь к Premiere Transcript JSON (от project root)
  - `segments[]` — массив сегментов клипа:
    - `id` — порядковый номер сегмента
    - `timecode` — HH:MM:SS таймкод начала
    - `start`, `end` — секунды (float)
    - `duration` — длительность сегмента в секундах
    - `words` — количество слов
    - `speaker` — имя спикера (из speakers.json)
    - `speaker_id` — идентификатор спикера (SPEAKER_XX)
    - `text` — полный текст сегмента
    - `confidence` — средняя confidence сегмента
    - `low_confidence` — true если confidence < 0.7
    - `use` — false по умолчанию, UXP-плагин устанавливает true для отобранных сегментов
    - `notes` — пустая строка по умолчанию, UXP-плагин заполняет аннотациями
...
### Terminal Output
    ...
    ════════════════════════════════════════════════════════════
      YTAI Transcribe v2.11                         (white on cyan BG)
    ────────────────────────────────────────────────────────────
      Project:    Interview
...
Per-stage time estimates calculated from `SPEED_COEFFICIENTS * total_duration`:
    ...
    SPEED_COEFFICIENTS = {
        ...
        "prproj": 0.001,        "lut_analysis": 0.002,
    }

Shown in: terminal Summary, XLSX Summary sheet, log file.
...
### LUT файлы не найдены
- ~/YTAI/luts/ отсутствует или пуст
- WARN LUT files not found, skipping LUT selection
- meta.json: "lut": null
- Остальные файлы генерируются нормально
...
### Полная цепочка обработки
    ...
    [Съёмка] -> Папка с клипами (C5090.MP4, C5091.MP4, ...)
                    ...
                    v
    ┌──────────────────────────────────────────────────────────┐
    │  Stage 02: transcribe_project v2.11  (ЭТОТ СКРИПТ)      │
    │                                                          │
    │  Вход:  папка с видео / один файл + число спикеров       │
    │  Выход: _transcription/ + xlsx + brief.json + .prproj + LUT │
    │         - per-clip: json, srt, txt, premiere_transcript   │
    │         - общие: combined_transcript, speakers, diarize  │
    │         - .prproj с клипами на V1, lut/ с выбранным LUT  │
    └────────────────────┬─────────────────────────────────────┘
                         |
           ┌─────────────┼──────────────┐
           v             v              v
...
### Один файл vs папка — разница в пайплайне
    ...
    LUT                          один на проект               один на клип
...
## Команды
    ...
    # Specific LUT
    python ... --project "/path/to/Interview" -n 2 --lut-name dark

    # Custom LUT file
    python ... --project "/path/to/Interview" -n 2 --lut "/path/to/my.cube"
