# transcribe_project v2.16 — Спецификация
...
**v2.16:** ETA и план во время выполнения этапов — `print_stage_header()`, `Spinner` с планом, `compute_drift()`.

**Скрипт:** ~/YTAI/scripts/02_transcribe/transcribe_project_v2.16.py
**Версия:** 2.16
...
## Окружение
    ...
    python ~/YTAI/scripts/02_transcribe/transcribe_project_v2.16.py \
...
### Вариант A: Папка с видео (flat)
    ...
    python transcribe_project_v2.16.py --project "/path/to/Interview" -y
...
### Вариант B: Один видеофайл
    ...
    python transcribe_project_v2.16.py --project "/path/to/C5090.MP4" -y
...
### Вариант C: Папка с подпапками камер (multi-camera)
    python transcribe_project_v2.16.py --project '/Volumes/RYA T7 Black/YTCG Gambling Ru' -y
...
### clip_offsets.json
    ...
    {
      "version": "2.16", "created_at": "2026-01-13T12:22:00Z",
      "clips": [
...
### diarization.json
    ...
    {
      "version": "2.16", "created_at": "2026-01-13T12:30:00Z",
...
**Слова НЕ включены** — доступны в per-clip JSON.
    ...
    {
      "version": "2.16", "created_at": "2026-01-13T12:50:47Z", "project": "Interview",
...
**Компоненты:**
- `progress_bar()` — Unicode прогресс-бар с градиентом цвета (cyan→yellow→green), ETA как `~Xm left` (cyan)
- `vlen()` — визуальная длина строки без ANSI-кодов
- `_trow(content, W, border)` — строка таблицы с гарантированным выравниванием бордюров
- `print_environment()` — блок окружения (Python, Torch, Whisper, Pyannote, Platform, venv)
- `compute_drift(st, est_times)` — drift factor (actual/estimated), clamped [0.3, 3.0]
- `print_stage_header(num, total, text, stage_key, est_times, st, str_set)` — расширенный заголовок этапа с планом и "что дальше"
- `class Spinner(text, plan_est=0)` — анимированный braille spinner с таймером; при `plan_est > 0` показывает план рядом с elapsed, delta на stop
...
### Terminal Output
    ...
    │  ──────────────────────────────────────────────────────────   │
    │  Python:     3.11.7                                          │
    │  Torch:      2.2.0 · MPS (Apple Silicon)                    │
    │  Whisper:    large-v3                                        │
    │  Pyannote:   3.1.1                                           │
    │  Platform:   macOS 15.3 · arm64                              │
    │  venv:       .venv_transcribe                                │
    ╰──────────────────────────────────────────────────────────────╯

    ...

    ========================================================
      YTAI Transcribe v2.16                         (white on cyan BG)
    --------------------------------------------------------
      Project:    Interview
...
Shows ALL 6 stages regardless of `--stages` filter. Uses `_trow()` for guaranteed border alignment.

    ┌──────────────────────────────────────────────────────────────┐
    │  YTAI_Edit                                                   │
    │  3 clips · 5:56          4/6 done · 3m 39s · ~1s left       │
    ├──────────────────────────────────────────────────────────────┤
    │     Stage             Plan    Actual    Delta                 │
    │  ──────────────────────────────────────────────────────────   │
    │  ✓  Extract audio      11s   →    1s   −10s                  │
    │  ✓  Diarization        53s   →   14s   −39s                  │
    │  ✓  Whisper          4m 09s  → 3m 24s  −45s                  │
...
- `·` — pending (dim), shows Plan estimate, drift-adjusted `~Xm` if drift > 15%
- `─` — skipped (not in this run, not completed)

**Header:** line 1 = project name (bold), line 2 = `clips · duration` + `N/M done · elapsed · ~Xm left`
**"~Xm left"** вместо "ETA ~Xm" — ясно что это оставшееся время (cyan вместо yellow)
**Drift-adjusted ETA:** ratio actual/estimated for completed stages (capped 0.3x–3.0x)
**Borders:** `_trow(content, W=62)` гарантирует выравнивание `│` при любой длине ANSI-строк

### In-Stage Display (v2.16)

**Stage Header** — `print_stage_header()` заменяет `print_stage()`. Показывает drift-adjusted план этапа + что идёт после:

      Stage 3/5  Whisper transcription (word_timestamps)
      ╸ Plan ~11m 24s  ·  After: 4→5→5b (~4s)

      Stage 2/5  Speaker diarization (54:19)
      ╸ Plan ~8m 08s  ·  After: 3→4→5→5b (~39m 22s)

      Stage 5/5  Generating outputs
      ╸ Plan ~1s  ·  Last stage

**Spinner с планом** — `Spinner(text, plan_est)`. Показывает план рядом с elapsed:

      ⠋ Diarizing 54:19 of audio  1m 36s / plan ~8m 08s

При завершении — delta от плана:

      ✓ Diarization complete (54:19)  1m 40s  (plan ~8m 08s, −6m 28s)

Формат delta: `+Xm Ys` если дольше, `−Xm Ys` если быстрее плана.

**Progress bar с планом** — Stages 1, 3 показывают план в extra:

      Stage 3 ████░░░░░░░░░  20%  ~9m left  1.55x · plan ~11m

**`compute_drift(st, est_times)`** — вынесен как утилита, используется в:
- `print_status_table()` — dashboard между этапами
- `print_stage_header()` — заголовок этапа
- Main loop — `ctx["_current_drift"]`, `ctx["_stage_plan_adj"]` для stage functions

**`ctx["_stage_plan_adj"]`** — drift-adjusted план текущего этапа, доступен внутри stage functions.

### Summary (print_summary)

Расширенная сводка после завершения всех стадий. Четыре секции в двойной рамке `╔═║╗`:

**1. Key Stats:**
- Total time с delta от estimate
- Speed: X.Xx realtime (duration video → processing time)
- Words, Segments, Speakers, Language
- Confidence (цветовая: green ≥0.85, yellow ≥0.75, red <0.75)
- Output paths

**2. Speakers breakdown:**
- Per-speaker: имя, duration (%), слова, confidence
- Gaps: время без речи (%)

**3. Stage breakdown:**
- Per-stage: Est vs Actual vs Delta, % от общего времени, bar chart

**4. Quality analysis (автоматический):**
- ✓/⚠ Confidence level (high/medium/low)
- ⚠ Low-confidence segments count
- ⚠ Per-speaker low confidence warnings
- ✓ Speaker balance analysis (main speaker / balanced dialogue)
- ⚠ Large gaps warning (>15%)
...
### Полная цепочка обработки
    ...
    [Съёмка] -> Папка с клипами (C5090.MP4, C5091.MP4, ...)
                    ...
                    v
    +----------------------------------------------------------+
    |  Stage 02: transcribe_project v2.16  (ЭТОТ СКРИПТ)      |
    |                                                          |
...
### v2.16
- **In-stage ETA:** план и сравнение с фактом ВНУТРИ этапа, а не только между этапами
- **`print_stage_header()`:** расширенный заголовок этапа — показывает drift-adjusted план + "что дальше" (After: 4→5→5b)
- **`Spinner(plan_est=)`:** показывает план рядом с elapsed (`1m 36s / plan ~8m 08s`), delta при завершении (`−6m 28s`)
- **`compute_drift()`:** вынесен как reusable утилита (из inline кода `print_status_table`)
- **Stage 1 progress bar:** добавлен `plan ~Xs` в extra
- **Stage 3 progress bar:** добавлен `plan ~Xm` в extra (`1.55x · plan ~11m`)
- **Stage 2, 4 Spinner:** передаётся `plan_est` для отображения плана в реальном времени
- **`ctx["_stage_plan_adj"]`:** drift-adjusted план текущего этапа, доступен stage functions
- **`ctx["_current_drift"]`:** текущий drift factor, вычисляется перед каждым этапом
- **Summary alignment fix:** Stage breakdown и Speakers с фиксированной шириной столбцов (`{:>8s}`, `{:>9s}`) — ровные колонки при любых значениях
- **Per-clip alignment fix:** Stage 1, Stage 3 per-clip строки с правым выравниванием: `fmt_duration:>6s`, `clip_words:>5`, `fmt_time:>7s`, `speed:>5.2f` — ровные колонки при разной длительности и количестве слов
- **Dry-run clip list:** `fmt_duration:>6s` для выравнивания длительности видеофайлов
- **Combined SRT:** `generate_combined_srt()` — объединённый SRT файл в главной папке рядом с JSON и XLSX. Для multi-camera — также per-camera SRT. Использует global timestamps (offset + local). Путь добавлен в `structure` блок `_transcript.json`

### v2.15
- **Environment block:** `print_environment()` — Python, Torch, Whisper, Pyannote, Platform, venv
- **Display overhaul:** переписаны `print_status_table` и `print_summary` с гарантированным выравниванием
- **`_trow()` + `vlen()`:** утилиты для строк таблиц с ANSI-кодами — правая граница `│` всегда ровная
- **ETA → "~Xm left":** вместо непонятного жёлтого "ETA ~Xm" — ясная надпись cyan-цветом
- **Заголовок таблицы:** 2 строки (проект + статистика), формат `N/M done · elapsed · ~Xm left`
- **Summary расширен:** processing speed, per-speaker breakdown (duration/words/confidence), quality analysis
- **Quality analysis:** автоматический анализ — confidence level, low-conf warnings, speaker balance, gap warnings
- **Per-speaker confidence:** `avg_confidence` per speaker в `build_combined_transcript()`
- **progress_bar:** `~Xm left` (cyan) вместо `⏱ Xm` (yellow)
