# transcribe_project v2.13 — Спецификация

## Обзор

Полная транскрипция: word-level timestamps, Premiere Transcript JSON, глобальная диаризация.
**v2.13** добавляет поддержку multi-camera проектов (подпапки камер) с per-camera выходными файлами.

**Скрипт:** ~/YTAI/scripts/02_transcribe/transcribe_project_v2.13.py
**Версия:** 2.13
...
## Окружение
    ...
    python ~/YTAI/scripts/02_transcribe/transcribe_project_v2.13.py \
...
### Вариант A: Папка с видео (flat)
    ...
    python transcribe_project_v2.13.py --project "/path/to/Interview" -y
...
### Вариант B: Один видеофайл
    ...
    python transcribe_project_v2.13.py --project "/path/to/C5090.MP4" -y
...
### Вариант C: Папка с подпапками камер (multi-camera) — NEW in v2.13

    python transcribe_project_v2.13.py --project '/Volumes/RYA T7 Black/YTCG Gambling Ru' -y
...
### 5.1c Структура файлов (Вариант C — multi-camera) — NEW in v2.13
...
### Per-camera output generation — NEW in v2.13
...
### clip_offsets.json
    ...
    {
      "version": "2.13", "created_at": "2026-01-13T12:22:00Z",
      "clips": [
...
### diarization.json
    ...
    {
      "version": "2.13", "created_at": "2026-01-13T12:30:00Z",
...
**Слова НЕ включены** — доступны в per-clip JSON.
    ...
    {
      "version": "2.13", "created_at": "2026-01-13T12:50:47Z", "project": "Interview",
...
### Terminal Output
    ...
    ========================================================
      YTAI Transcribe v2.13                         (white on cyan BG)
    --------------------------------------------------------
      Project:    Interview
...
### Полная цепочка обработки
    ...
    [Съёмка] -> Папка с клипами (C5090.MP4, C5091.MP4, ...)
                    ...
                    v
    +----------------------------------------------------------+
    |  Stage 02: transcribe_project v2.13  (ЭТОТ СКРИПТ)      |
    |                                                          |
...
- Replaced brief.json with `{project}_transcript.json` (originally `_project.json`, renamed in v2.13) — comprehensive project manifest
