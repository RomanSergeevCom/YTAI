...
def parse_args():
    parser = argparse.ArgumentParser(
        description="YTAI Transcribe v2.15 — Word-level transcription with speaker diarization"
    )
...
def print_summary(ctx, st):
    ...
    if speaker_stats:
        ...
        for label, sp_info in sorted(speaker_stats.items(), key=lambda x: -x[1].get("duration", 0)):
            ...
            row = (f"  {name:<14s} {fmt_duration(dur):>5s} ({pct:4.1f}%)"
                   f"  {words:>5,} words  {conf_c}{sp_conf:.2f}{C.RESET} conf")
            print(_trow(row, W, B))
        # Gaps
        if gap_dur > 0 and total_dur > 0:
            gap_pct = gap_dur / total_dur * 100
            print(_trow(f"  {C.DIM}{'Gaps':<14s} {fmt_duration(gap_dur):>5s} ({gap_pct:4.1f}%){C.RESET}", W, B))

    # ── Stage breakdown ──
    print(f"  {C.DIM}╠{'═' * W}╣{C.RESET}")
    if est_times:
        hdr = f"  {C.DIM}{'Stage':<16s} {'Est':>6s} {'Actual':>7s} {'Δ':>7s} {'%':>4s}  Bar{C.RESET}"
        print(_trow(hdr, W, B))
        print(_trow(f"  {C.DIM}{'─' * 56}{C.RESET}", W, B))
    else:
        hdr = f"  {C.DIM}{'Stage':<16s} {'Time':>7s} {'%':>4s}  Bar{C.RESET}"
        print(_trow(hdr, W, B))
        print(_trow(f"  {C.DIM}{'─' * 56}{C.RESET}", W, B))
    ...
    for k, label in STAGE_LABELS:
        ...
        if est_times:
            ...
            d_sign = "+" if delta >= 0 else "−"
            delta_txt = f"{d_sign}{fmt_time(abs(delta))}"
            row = (f"  {label:<16s} {fmt_time(e):>6s} {fmt_time(t):>7s} "
                   f"{d_color}{delta_txt:>7s}{C.RESET} "
                   f"{pct:3.0f}%  {color}{bar}{C.RESET}")
        else:
            row = f"  {label:<16s} {fmt_time(t):>7s} {pct:3.0f}%  {color}{bar}{C.RESET}"
        print(_trow(row, W, B))
