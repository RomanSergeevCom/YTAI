...
LUTS_DIR = Path.home() / "YTAI" / "luts"
LUT_NAMES = ["dark", "normal", "bright"]

LUT_THRESHOLD_DARK = 0.35
LUT_THRESHOLD_BRIGHT = 0.55
...
def progress_bar(current, total, width=30, label="", eta_str="", extra=""):
    ...
    if eta_str:
        parts.append(f"  {C.YELLOW}{C.BOLD}⏱ {eta_str}{C.RESET}")
    if extra:
...
STAGE_LABELS = [
    ...
    ("5b", "Premiere proj"), ("5c", "LUT select"),
]

def print_status_table(st, est_times, str_set):
    """Print a compact plan-vs-fact table after each stage completes."""
    if not est_times:
        return
    total_actual = sum(st.values())
    total_est = sum(est_times.get(k, 0) for k in str_set)
    ...
    for k, label in STAGE_LABELS:
        ...
        if k in st:
            ...
            print(f"  {C.DIM}│{C.RESET}  {C.GREEN}✓{C.RESET} {label:<14s}"
                  f"  {C.DIM}est{C.RESET} {fmt_time(e):>5s}"
                  f"  → {fmt_time(t):>6s}"
                  f"  {d_color}{d_sign}{fmt_time(abs(delta)):>5s}{C.RESET}"
                  f"  {C.DIM}│{C.RESET}")
        else:
            print(f"  {C.DIM}│  · {label:<14s}"
                  f"  est {fmt_time(e):>5s}"
                  f"{'':>15s}│{C.RESET}")
    # Total line
    delta = total_actual - total_est
    d_color = C.RED if delta > 0 else C.GREEN
    d_sign = "+" if delta >= 0 else "-"
    remaining = total_est - total_actual
    print(f"  {C.DIM}├{'─' * 52}┤{C.RESET}")
    print(f"  {C.DIM}│{C.RESET}  {C.BOLD}Σ{C.RESET} {fmt_time(total_actual):>6s} / {fmt_time(total_est)}"
          f"  {d_color}{d_sign}{fmt_time(abs(delta))}{C.RESET}"
          f"  {C.DIM}remaining ~{fmt_time(max(remaining, 0))}{C.RESET}"
          f"  {C.DIM}│{C.RESET}")
    print(f"  {C.DIM}└{'─' * 52}┘{C.RESET}")
...
class Spinner:
    ...
    def _spin(self):
        ...
        while not self._stop.is_set():
            ...
            line = f"  {C.CYAN}{frame}{C.RESET} {self.text}  {C.DIM}{fmt_time(elapsed)}{C.RESET}"
    ...
    def stop(self, final_text=None):
        ...
        if final_text:
            sys.stdout.write(f"{C.CLEAR_LINE}  {C.GREEN}{C.BOLD}✓{C.RESET} {final_text}  "
                             f"{C.DIM}{fmt_time(elapsed)}{C.RESET}\n")
        else:
...
def find_videos(folder):
    """Find video files in folder (non-recursive, skip hidden)."""
    return sorted([
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="YTAI Transcribe v2.11 — Word-level transcription with speaker diarization"
    )
    ...
    parser.add_argument("--lut", default=None, help="Path to specific LUT file")
    parser.add_argument("--lut-name", default=None, choices=LUT_NAMES, help="LUT name (skip analysis)")
    parser.add_argument("--no-lut", action="store_true", help="Skip LUT selection")
...
def preflight(ctx, args):
    ...
    if ctx["transcription_dir"].exists():
        ...
        if not args.y and not args.resume:
    ...
    # 11. LUT files
    has_luts = LUTS_DIR.exists() and any(LUTS_DIR.glob("*.cube"))
    if has_luts:
        print(f"  {C.GREEN}✓{C.RESET} LUT files")
    elif not args.no_lut:
        print_warn("LUT files not found, will skip LUT selection")
    ...
    stage_estimates = {
        ...
        "5c": total_duration * SPEED_COEFFICIENTS["lut_analysis"],
    }
    ...
    for i, vf in enumerate(clips):
        ...
        if len(clips) > 1:
            ...
            sys.stdout.write(C.CLEAR_LINE + progress_bar(
                ...
                label=f"[{i+1}/{len(clips)}]", eta_str=eta_str))
            sys.stdout.flush()
        ...
        if len(clips) > 1:
            ...
            sys.stdout.flush()
        print(f"  {clip_id} -> {clip_id}_audio.wav  {fmt_duration(duration)}  "
              f"{C.GREEN}done{C.RESET}  {fmt_time(elapsed)}")
...
def stage3_whisper(ctx, args, resources):
    ...
    for i, vf in enumerate(clips):
        clip_id = vf.stem
        wav_path = td / "per_clip" / clip_id / f"{clip_id}_audio.wav"
        checkpoint_path = td / "per_clip" / clip_id / f"{clip_id}_whisper_raw.json"
        ...
        if checkpoint_path.exists():
            try:
                ...
                if len(clips) > 1 and i > 0:
                    sys.stdout.write(C.CLEAR_LINE)
                    sys.stdout.flush()
                print(f"  [{i+1:2d}/{len(clips)}] {clip_id}  {fmt_duration(dur)}  "
                      f"{clip_words} words  {C.CYAN}cached{C.RESET}")
                continue
            except (json.JSONDecodeError, KeyError):
                pass  # Corrupted checkpoint, re-transcribe

        spinner = Spinner(f"[{i+1}/{len(clips)}] {clip_id}  {fmt_duration(dur)}  transcribing")
        spinner.start()
        ...
        print(f"  [{i+1:2d}/{len(clips)}] {clip_id}  {fmt_duration(dur)}  "
              f"{clip_words} words  {status}  {fmt_time(elapsed_clip)}  "
              f"{C.DIM}({speed:.2f}x){C.RESET}")
        ...
        if len(clips) > 1:
            ...
            bar_line = progress_bar(
                ...
                extra=f"{avg_speed:.2f}x"
            )
            sys.stdout.write(C.CLEAR_LINE + bar_line)
            sys.stdout.flush()

            set_terminal_title(f"YTAI > Stage 3/5 > {pct:.0f}% > ETA {fmt_time(eta_sec)}")
...
def generate_xlsx(ctx, args):
    ...
    for clip_idx, vf in enumerate(ctx["video_files"]):
        cid = vf.stem
    ...
    # ── Sheet 3: Brief (for marking segments to use in editing) ──
    ws3 = wb.create_sheet("Brief")
    brief_headers = ["Clip", "Timecode", "Duration", "Words", "Text", "Use", "Notes"]
    ...
    for vf in ctx["video_files"]:
        ...
        # Clip separator
        sep_text = f"{cid}  |  {fmt_duration(clip_dur)}  |  {clip_words} words"
        for c in range(1, len(brief_headers) + 1):
            ws3.cell(row=br, column=c).fill = clip_fill
        ws3.cell(row=br, column=1, value=sep_text).font = clip_font
        ws3.merge_cells(start_row=br, start_column=1, end_row=br, end_column=len(brief_headers))
        br += 1

        if not tr["segments"]:
            ws3.cell(row=br, column=1, value=cid)
            ws3.cell(row=br, column=2, value=fmt_timecode(co))
            ws3.cell(row=br, column=5, value="[no speech]")
            br += 1
            continue
        ...
        for seg in tr["segments"]:
            ...
            ws3.cell(row=br, column=1, value=cid)
            ws3.cell(row=br, column=2, value=fmt_timecode(gs))
            ws3.cell(row=br, column=3, value=fmt_duration(dur))
            ws3.cell(row=br, column=4, value=wc)
            tc3 = ws3.cell(row=br, column=5, value=seg["text"])
            tc3.alignment = wrap_align
            # Use column (6) left empty for user
            # Notes: auto-fill for low confidence
            if seg["low_confidence"]:
                ws3.cell(row=br, column=7, value=f"LOW conf ({seg['avg_confidence']:.2f})")
                tc3.fill = yf
            for c in range(1, len(brief_headers) + 1):
                ws3.cell(row=br, column=c).border = thin_border
            br += 1

    ws3.freeze_panes = "A2"
    brief_widths = {"A": 10, "B": 12, "C": 10, "D": 7, "E": 80, "F": 6, "G": 25}
    for letter, w in brief_widths.items():
        ws3.column_dimensions[letter].width = w

    # ── Sheet 4: Media ──
    ws4 = wb.create_sheet("Media")
    ...
    for vf in ctx["video_files"]:
        ...
        ws4.cell(row=mr, column=1, value=vf.stem)
        ws4.cell(row=mr, column=2, value=vf.name)
        ws4.cell(row=mr, column=3, value=fmt_duration(dur))
        ws4.cell(row=mr, column=4, value=res_str)
        ws4.cell(row=mr, column=5, value=fps)
        ws4.cell(row=mr, column=6, value=frames)
        ws4.cell(row=mr, column=7, value=codec_str)
        ws4.cell(row=mr, column=8, value=round(vbr, 1))
        ws4.cell(row=mr, column=9, value=pix_str)
        ws4.cell(row=mr, column=10, value=audio_str)
        ws4.cell(row=mr, column=11, value=size_gb)
        ws4.cell(row=mr, column=12, value=created_str)
        ws4.cell(row=mr, column=13, value=tc)
        for c in range(1, len(media_headers) + 1):
            ws4.cell(row=mr, column=c).border = thin_border
        mr += 1

    # Total row
    bf = Font(bold=True)
    total_dur = sum(clip_media.get(vf.stem, {}).get("duration", 0) for vf in ctx["video_files"])
    ws4.cell(row=mr, column=1, value="TOTAL").font = bf
    ws4.cell(row=mr, column=3, value=fmt_duration(total_dur)).font = bf
    ws4.cell(row=mr, column=6, value=total_frames).font = bf
    ws4.cell(row=mr, column=11, value=round(total_size / (1024 ** 3), 2)).font = bf
    for c in range(1, len(media_headers) + 1):
        ws4.cell(row=mr, column=c).border = thin_border
    ...
    for letter, mw in media_widths.items():
        ws4.column_dimensions[letter].width = mw
...
def generate_combined_transcript(ctx):
    ...
    for vf in ctx["video_files"]:
        ...
        for seg in tr["segments"]:
            ...
            if sp not in spa: spa[sp] = {"segments":0,"words":0,"duration":0.0}
    ...
    for sp in spa: spa[sp]["duration"] = round(spa[sp]["duration"], 3)
...
def generate_brief_json(ctx, args):
    """Generate brief.json for UXP plugin import."""
    ...
    for vf in ctx["video_files"]:
        ...
        clips_out.append({
            ...
            "premiere_transcript": f"per_clip/{cid}/{cid}_premiere_transcript.json",
            "segments": segs_out,
        })
    brief = {
        "version": VERSION,
    ...
    brief_path = ctx["work_dir"] / f"{ctx['project_name']}_brief.json"
...
def stage5_generate_outputs(ctx, args):
    ...
    for idx, vf in enumerate(clips):
        cid = vf.stem; tr = ctx["clip_transcripts"].get(cid)
        if not tr: continue
        cd = td/"per_clip"/cid; cd.mkdir(parents=True, exist_ok=True)
    ...
    brief_path = generate_brief_json(ctx, args); print(f"  {brief_path.name}")
...
def analyze_exposure(video_path, num_frames=10):
    duration = get_duration(video_path)
    timestamps = np.linspace(1, max(duration-1, 2), num_frames)
    brightnesses = []
    for ts in timestamps:
        cmd = ["ffmpeg","-ss",str(ts),"-i",str(video_path),
               "-frames:v","1","-f","rawvideo","-pix_fmt","gray",
               "-s","320x180","-v","quiet","pipe:1"]
        raw = subprocess.run(cmd, capture_output=True).stdout
        if raw:
            frame = np.frombuffer(raw, dtype=np.uint8)
            brightnesses.append(np.mean(frame) / 255.0)
    return float(np.median(brightnesses)) if brightnesses else 0.5


def select_lut(brightness):
    if brightness < LUT_THRESHOLD_DARK: return "dark"
    elif brightness > LUT_THRESHOLD_BRIGHT: return "bright"
    return "normal"


def stage5c_lut_selection(ctx, args, resources):
    if args.no_lut: return 0.0
    if args.lut:
        lut_path = Path(args.lut)
        if not lut_path.exists():
            print_warn(f"LUT file not found: {args.lut}"); return 0.0
        t0 = time.time()
        ld = ctx["transcription_dir"]/"lut"; ld.mkdir(exist_ok=True)
        shutil.copy2(str(lut_path), str(ld/lut_path.name))
        shutil.copy2(str(lut_path), str(ctx["work_dir"]/lut_path.name))
        ctx["lut_info"] = {"name": lut_path.stem, "brightness": None}
        elapsed = time.time()-t0
        print(f"  {C.CYAN}[5c]{C.RESET} Custom LUT: {lut_path.name}  {C.GREEN}done{C.RESET}  {fmt_time(elapsed)}")
        return elapsed
    if not resources.get("has_luts") and not args.lut_name:
        print_warn("LUT files not found, skipping LUT selection")
        ctx["lut_info"] = None; return 0.0
    t0 = time.time(); set_terminal_title("YTAI > [5c] > LUT analysis")
    if args.lut_name:
        lut_name = args.lut_name; brightness = None
    else:
        brightness = analyze_exposure(ctx["video_files"][0])
        lut_name = select_lut(brightness)
    lut_src = LUTS_DIR / f"{lut_name}.cube"
    if not lut_src.exists():
        print_warn(f"LUT not found: {lut_src}"); ctx["lut_info"] = None; return 0.0
    ld = ctx["transcription_dir"]/"lut"; ld.mkdir(exist_ok=True)
    shutil.copy2(str(lut_src), str(ld/lut_src.name))
    shutil.copy2(str(lut_src), str(ctx["work_dir"]/lut_src.name))
    li = {"selected": lut_name,
        "median_brightness": round(brightness,4) if brightness else None,
        "thresholds": {"dark": LUT_THRESHOLD_DARK, "bright": LUT_THRESHOLD_BRIGHT},
        "source_clip": ctx["video_files"][0].name, "frames_analyzed": 10,
        "all_luts": [f"{n}.cube" for n in LUT_NAMES]}
    with open(ld/"lut_info.json","w") as f: json.dump(li,f,indent=2,ensure_ascii=False)
    ctx["lut_info"] = {"name": lut_name, "brightness": brightness}
    br = f"brightness {brightness:.2f} -> " if brightness is not None else ""
    elapsed = time.time()-t0
    print(f"  {C.CYAN}[5c]{C.RESET} LUT analysis: {br}{lut_name}.cube  {C.GREEN}done{C.RESET}  {fmt_time(elapsed)}")
    if brightness is not None:
        print(f"  Recommended LUT: {lut_name} (brightness: {brightness:.2f})")
    return elapsed
...
def save_meta(ctx, args, resources, st):
    ...
    if ctx.get("lut_info"):
        li = ctx["lut_info"]
        lv = {"name": li["name"], "brightness": round(li["brightness"],4) if li["brightness"] else None}
    comp = [k for k in ["1","2","3","4","5","5b","5c"] if k in st]
    ...
    meta = {"version": VERSION, "status": status,
        ...
        "stages_timing": {"1_extract_audio": round(st.get("1",0),1),
            ...
            "5b_premiere_project": round(st.get("5b",0),1), "5c_lut_selection": round(st.get("5c",0),1)},
        "lut": lv, "total_time": round(sum(st.values()),1)}
...
def print_summary(ctx, st):
    ...
    print(f"  Total time:   {C.BOLD}{fmt_time(total)}{C.RESET}", end="")
    if total_est > 0:
        delta = total - total_est
        d_color = C.RED if delta > 0 else C.GREEN
        d_sign = "+" if delta > 0 else "-"
        print(f"  {C.DIM}(est: {fmt_time(total_est)}, "
              f"{d_color}{d_sign}{fmt_time(abs(delta))}{C.RESET}{C.DIM}){C.RESET}")
    else:
        print()
    print(f"  Words:        {cs.get('total_words', 0):,}")
    print(f"  Segments:     {cs.get('total_segments', 0)}")
    print(f"  Speakers:     {len(ctx.get('speaker_map', {}))}")
    print(f"  Language:     {ctx.get('main_language', '?')}")
    print(f"  Confidence:   {cs.get('avg_confidence', 0):.2f}"
          f"  {C.DIM}({cs.get('low_confidence_segments', 0)} low){C.RESET}")
    print(f"  Output:       {ctx['transcription_dir'].name}/")
    print(f"                {ctx['project_name']}_transcript.xlsx")

    print(f"\n  {'─' * 62}")
    if est_times:
        print(f"  {'Stage':<16s} {'Est':>6s}  {'Actual':>8s}  {'Δ':>7s}  {'%':>4s}  Bar")
        print(f"  {'─' * 16} {'─' * 6}  {'─' * 8}  {'─' * 7}  {'─' * 4}  {'─' * 16}")
    else:
        print(f"  {'Stage':<16s} {'Time':>8s}  {'%':>4s}  Bar")
        print(f"  {'─' * 16} {'─' * 8}  {'─' * 4}  {'─' * 16}")
    ...
    for k, label in [("1", "Extract audio"), ("2", "Diarization"), ("3", "Whisper"),
        ...
        if t == 0 and k not in st:
            continue
        pct = t / total * 100 if total > 0 else 0
        bar = summary_bar(t, max_stage_time, width=16)
        ...
        if est_times:
            ...
            d_color = C.RED if delta > e * 0.3 else (C.GREEN if delta < -e * 0.3 else C.DIM)
            d_sign = "+" if delta >= 0 else "-"
            print(f"  {label:<16s} {fmt_time(e):>6s}  {fmt_time(t):>8s}  "
                  f"{d_color}{d_sign}{fmt_time(abs(delta)):>5s}{C.RESET}  "
                  f"{pct:3.0f}%  {color}{bar}{C.RESET}")
        else:
            print(f"  {label:<16s} {fmt_time(t):>8s}  {pct:3.0f}%  {color}{bar}{C.RESET}")
...
def try_resume(ctx, args):
    ...
    for s in ["1","2","3","4","5","5b","5c"]:
        if s not in completed:
            ...
            if "4" in completed and s in ("5", "5b", "5c"):
                ct = {}
                for vf in ctx["video_files"]:
                    jp = td/"per_clip"/vf.stem/f"{vf.stem}_transcript.json"
                    if jp.exists():
                        try:
...
def main():
    ...
    if args.dry_run:
        str_set = set(args.stages.split(",")) if args.stages else {"1","2","3","4","5","5b","5c"}
        ...
        for vf in ctx["video_files"]:
            ...
            vc = info.get("video_codec", "").upper()
            print(f"    {vf.name:<20s}  {fmt_duration(dur)}  {res} {fps} {vc}")
    ...
    for s in ["1","2","3","4","5","5b","5c"]:
        ...
        pipeline_prog.stage_started(s)
        if s == "1":
            print_stage("1","5","Extracting audio")
            st["1"] = stage1_extract_audio(ctx, args)
        elif s == "2":
            print_stage("2","5",f"Speaker diarization ({fmt_duration(ctx['total_duration'])})")
            st["2"] = stage2_diarization(ctx, args, resources)
        elif s == "3":
            print_stage("3","5","Whisper transcription (word_timestamps)")
            st["3"] = stage3_whisper(ctx, args, resources)
        elif s == "4":
            print_stage("4","5","Mapping speakers")
            st["4"] = stage4_speaker_mapping(ctx, args)
        elif s == "5":
            print_stage("5","5","Generating outputs")
            st["5"] = stage5_generate_outputs(ctx, args)
        elif s == "5b":
            st["5b"] = stage5b_generate_prproj(ctx, args, resources)
        elif s == "5c":
            st["5c"] = stage5c_lut_selection(ctx, args, resources)
        pipeline_prog.stage_completed(s)
        save_meta(ctx, args, resources, st)
        print_status_table(st, ctx.get("stage_estimates", {}), str_set)
    pipeline_prog.final_bar()
