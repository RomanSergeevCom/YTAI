...
def print_status_table(st, est_times, str_set, current_stage=None, ctx=None):
    ...
    Shows: header with project info + progress counter, per-stage plan/fact
    with status icons (✓/▶/·), drift-adjusted ETA, and progress bar.
    """
    if not est_times:
        return
    # Show ALL stages (full picture), not just str_set
    all_stages = list(STAGE_LABELS)
    n_total = len(all_stages)
    n_done = sum(1 for k, _ in all_stages if k in st)
    n_active = sum(1 for k, _ in all_stages if k in str_set)

    total_actual = sum(st.values())
    total_est = sum(est_times.get(k, 0) for k in [k for k, _ in all_stages])

    # Drift factor: how actual compares to estimates for completed stages
    # Only use stages with meaningful estimates (> 1s) for drift calc
    est_done = sum(est_times.get(k, 0) for k in st if est_times.get(k, 0) > 1)
    act_done = sum(st[k] for k in st if est_times.get(k, 0) > 1)
    drift = min(max(act_done / est_done, 0.3), 3.0) if est_done > 1 else 1.0

    # ETA: remaining estimate adjusted by drift (for stages still to run)
    ...
    info_parts = []
    if proj:
        info_parts.append(f"{C.BOLD}{proj}{C.RESET}")
    if n_clips:
        info_parts.append(f"{n_clips} clips")
    if dur_str:
        info_parts.append(dur_str)

    # Header
    print(f"  {C.DIM}┌{'─' * W}┐{C.RESET}")
    status_str = f"{n_done}/{n_total} done"
    elapsed_str = f"Elapsed {fmt_time(total_actual)}"
    eta_str = f"ETA ~{fmt_time(max(eta, 0))}" if remaining_keys else "Done"
    header_info = " │ ".join(info_parts) if info_parts else ""
    header_right = f"{status_str}  {C.DIM}│{C.RESET}  {elapsed_str}  {C.DIM}│{C.RESET}  {C.YELLOW}{eta_str}{C.RESET}"
    print(f"  {C.DIM}│{C.RESET}  {header_info}  {header_right}")

    # Stage table header
    print(f"  {C.DIM}├{'─' * W}┤{C.RESET}")
    print(f"  {C.DIM}│{C.RESET}  {C.DIM}{'':>3s}{'Stage':<17s}{'Plan':>7s}{'Actual':>9s}{'Delta':>9s}{'':>3s}{C.RESET}{C.DIM}│{C.RESET}")
    print(f"  {C.DIM}│{'─' * W}│{C.RESET}")
    ...
    for k, label in all_stages:
        e = est_times.get(k, 0)
        if k in st:
            # Completed stage (either current run or restored from resume)
            t = st[k]
            delta = t - e
            d_color = C.RED if delta > e * 0.3 else (C.GREEN if delta < -e * 0.3 else C.DIM)
            d_sign = "+" if delta >= 0 else "-"
            icon = f"{C.GREEN}✓{C.RESET}"
            actual_str = fmt_time(t)
            delta_str = f"{d_color}{d_sign}{fmt_time(abs(delta)):>5s}{C.RESET}"
            print(f"  {C.DIM}│{C.RESET}  {icon}  {label:<16s}"
                  f"{fmt_time(e):>7s}"
                  f"  →{actual_str:>7s}"
                  f"  {delta_str}"
                  f"{'':>3s}{C.DIM}│{C.RESET}")
            ...
            print(f"  {C.DIM}│{C.RESET}  {icon}  {C.BOLD}{label:<16s}{C.RESET}"
                  f"{fmt_time(e):>7s}"
                  f"  {C.CYAN}running...{C.RESET}"
                  f"{'':>9s}{C.DIM}│{C.RESET}")
        elif k in str_set:
            # Pending stage (will run)
            icon = f"{C.DIM}·{C.RESET}"
            adj_est = e * drift
            adj_str = f"~{fmt_time(adj_est)}" if abs(drift - 1.0) > 0.15 else ""
            print(f"  {C.DIM}│  {icon}  {label:<16s}"
                  f"{fmt_time(e):>7s}"
                  f"{'':>9s}{'':>9s}"
                  f"{adj_str:>5s}│{C.RESET}")
        else:
            # Skipped stage (not in this run, not completed)
            print(f"  {C.DIM}│  ─  {label:<16s}"
                  f"{'skip':>7s}"
                  f"{'':>22s}│{C.RESET}")

    # Footer: progress bar + totals
    pct = n_done / n_total * 100 if n_total > 0 else 0
    bar_width = W - 30
    bar_filled = int(bar_width * pct / 100)
    bar_str = f"{C.GREEN}{'█' * bar_filled}{C.RESET}{C.DIM}{'░' * (bar_width - bar_filled)}{C.RESET}"
    print(f"  {C.DIM}├{'─' * W}┤{C.RESET}")
    # Total line with plan vs fact
    if total_est > 0:
        total_delta = total_actual - total_est
        td_color = C.RED if total_delta > 0 else C.GREEN
        td_sign = "+" if total_delta >= 0 else "-"
        print(f"  {C.DIM}│{C.RESET}  {bar_str}  {C.BOLD}{pct:3.0f}%{C.RESET}"
              f"  {C.DIM}Σ{C.RESET} {fmt_time(total_actual)}/{fmt_time(total_est)}"
              f" {td_color}{td_sign}{fmt_time(abs(total_delta))}{C.RESET}"
              f"  {C.DIM}│{C.RESET}")
    print(f"  {C.DIM}└{'─' * W}┘{C.RESET}")
...
def generate_project_json(ctx, args):
    """Generate {project}_project.json — central project manifest with all data."""
    ...
    for vf in ctx["video_files"]:
        ...
        # File paths (relative to transcription_dir)
        files_block = {
            "premiere_transcript": f"per_clip/{cid}/{cid}_premiere_transcript.json",
            "transcript": f"per_clip/{cid}/{cid}_transcript.json",
            "srt": f"per_clip/{cid}/{cid}_transcript.srt",
            "txt": f"per_clip/{cid}/{cid}_transcript.txt",
        }
        # Segments
        segs_out = []
        for seg in tr["segments"]:
    ...
    project_path = ctx["work_dir"] / f"{ctx['project_name']}_project.json"
