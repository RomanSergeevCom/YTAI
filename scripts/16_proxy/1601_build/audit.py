...
def media_of(root):
    ...
    for cur, dirs, files in os.walk(root):
        ...
        for f in files:
            ...
            # ⚠️ Симлинки НЕ выбрасываем. Раньше здесь стоял `continue`, и на
            # съёмочных днях, собранных day_ingest, аудит был слеп целиком:
            # в `01_Source` там не файлы, а ссылки на карту, и «проверь прокси»
            # по YTEVO03 показывало 0 клипов вместо 172 — то есть отвечало
            # «всё хорошо» там, где не проверило ничего. Битая ссылка
            # отсеивается ниже сама: probe на ней не откроется.
