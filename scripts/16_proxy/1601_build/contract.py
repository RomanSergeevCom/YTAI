...
def probe(path, count_frames=True, check_audio=True, timeout=1800):
    ...
    # таймкод: Sony держит его на data-дорожке (`rtmd`), а не в format_tags —
    # `format_tags=timecode` у оригинала вернёт пусто. У готовой прокси ffmpeg
    # кладёт его в `tmcd` и дублирует в format_tags, поэтому читаем оба места.
    timecode, tc_src = "", ""
    for s in streams:
...
def decide(spec, bitrate=DEFAULT_BITRATE, chroma="420", force=None):
    ...
    """
    if spec.is_symlink and force != "encode":
        return Decision("skip-symlink",
                        "клип-спаннер живёт симлинком во второй сцене — "
