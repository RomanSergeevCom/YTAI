...
class Store:
    ...
    def release_all_leases(self):
        """Снять все лизы разом. Зовётся на СТАРТЕ прогона.

        Лиза живёт три часа — этого хватает самому долгому клипу, но убитый
        `kill -9` прогон оставляет свои лизы висеть, и при перезапуске те же
        клипы три часа невидимы для очереди: `status` показывает «в работе 25»,
        хотя не работает никто. Ждать истечения незачем — единственность
        экземпляра уже гарантирована pid-файлом, значит на старте чужих живых
        лиз не бывает по определению. Попытку не тратим: клип не виноват.
        """
        with self._lock:
            stuck = [r for r, c in self.clips.items() if c["state"] == "lease"]
            for r in stuck:
                self._append({"kind": "release", "rel": r})
                self._apply({"kind": "release", "rel": r})
            return len(stuck)
