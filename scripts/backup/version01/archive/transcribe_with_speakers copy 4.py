...
v3 fixes:
    - PyAnnote API: use_auth_token → token
    - Fixed language detection

Usage:
    python transcribe_with_speakers.py --project "/Volumes/RYA Blue/YTCG37_Hadi_Dawani" --language en
    python transcribe_with_speakers.py --project "/Volumes/RYA Blue/YTCG37_Hadi_Dawani" --language en --num-speakers 2
...
# Whisper models (public MLX models - no auth required)
WHISPER_MODELS = {
    "en": "mlx-community/whisper-large-v3-turbo",
    "ar": "mlx-community/whisper-large-v3-mlx",
    "multi": "mlx-community/whisper-large-v3-mlx",
}
...
def run_speaker_diarization(
    ...
    try:
        # NEW API: use 'token' instead of 'use_auth_token'
        ...
        tee_print(log_f, "  1. Accept license: https://huggingface.co/pyannote/speaker-diarization-3.1")
        tee_print(log_f, "  2. Accept license: https://huggingface.co/pyannote/segmentation-3.0")
        tee_print(log_f, "  3. Verify token: huggingface-cli whoami")
        tee_print(log_f, "  4. Re-login: huggingface-cli login")
        return []


def transcribe_audio(audio_path: Path, language: str, log_f) -> dict:
...
def analyze_speakers(segments: list[dict]) -> list[dict]:
    """Analyze speakers and generate stats."""
    speaker_stats = {}
...
def main() -> None:
    ...
    with log_path.open("w", encoding="utf-8") as log_f:
        tee_print(log_f, "=" * 60)
        tee_print(log_f, "TRANSCRIBE WITH SPEAKERS v3")
        tee_print(log_f, "=" * 60)
        ...
        tee_print(log_f, f"HF Token    : {'✓' if hf_token else '✗'}")
        tee_print(log_f, "")
        ...
        if args.skip_diarization:
            tee_print(log_f, "PHASE 1: Diarization SKIPPED")
            ...
            tee_print(log_f, "PHASE 1: Diarization SKIPPED (no HF token)")
        else:
            ...
            diarization_segments = run_speaker_diarization(
                full_audio, hf_token, log_f, args.num_speakers
            )
