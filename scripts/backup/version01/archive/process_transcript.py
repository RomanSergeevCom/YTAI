...
Output:
    ...
    ├── *_named.json (with speaker names)
    ├── *_named.txt
    └── *_by_clips.xlsx (table: file | timecode | speaker | text)
...
OLLAMA_TIMEOUT = 600  # 10 minutes for 70B model
...
def extract_speaker_names_llm(segments: List[dict], speakers: Set[str], logger: logging.Logger) -> Dict[str, str]:
    ...
    # Build sample (first 5 lines per speaker, max 100 chars)
    transcript_parts = []
...
Examples:
    ...
    if not args.skip_llm and not speaker_map:
        llm_names = extract_speaker_names_llm(segments, original_speakers, logger)
    ...
    logger.info(f"  {paths['transcripts_clean_dir']}/*.srt")
