...
WHISPER_MODELS = {
    ...
    "multi": "mlx-community/whisper-large-v3",         # Multi-language (auto-detect)
}
...
def run_speaker_diarization(
    ...
    try:
        ...
        # Preload WAV into memory to bypass torchcodec/ffmpeg decoding issues.
        # Works for real .wav inputs.
        try:
            import soundfile as sf
            import numpy as np
            import torch

            wav, sr = sf.read(str(audio_path), always_2d=True)  # (time, channels)
            wav = wav.T.astype(np.float32)  # (channels, time)
            audio_in_memory = {"waveform": torch.from_numpy(wav), "sample_rate": int(sr)}
        except Exception:
            import wave
            import numpy as np
            import torch
            ...
            audio = audio.reshape(-1, ch).T  # (channels, time)
            audio_in_memory = {"waveform": torch.from_numpy(audio), "sample_rate": int(sr)}

        # Run diarization
        if num_speakers:
            diarization = pipeline(audio_in_memory, num_speakers=num_speakers)
        else:
...
def transcribe_audio(
    ...
    try:
        result = mlx_whisper.transcribe(
            ...
            language=None if language in ("auto", "multi") else language,
...
Examples:
    ...
    ap.add_argument("--language", choices=["en", "ar", "auto", "multi"], default="auto",
