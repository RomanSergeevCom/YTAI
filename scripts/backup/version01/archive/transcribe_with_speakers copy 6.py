...
v5 - Uses OpenAI Whisper (not MLX) + PyAnnote diarization

Usage:
    python transcribe_with_speakers.py --project "/Volumes/RYA Blue/YTCG37_Hadi_Dawani"
    python transcribe_with_speakers.py --project "/Volumes/RYA Blue/YTCG37_Hadi_Dawani" --num-speakers 2
...
def run_speaker_diarization(
    ...
    try:
        # No token - uses cached credentials
        ...
        tee_print(log_f, "  3. Run: python -c \"from huggingface_hub import login; login()\"")
        return []


def transcribe_audio(audio_path: Path, model, log_f) -> dict:
    """Transcribe audio using OpenAI Whisper."""
    tee_print(log_f, f"  Transcribing: {audio_path.name}")
...
def analyze_speakers(all_segments: list[dict]) -> list[dict]:
    ...
    for seg in all_segments:
        ...
        if len(text) > 20 and len(speaker_stats[speaker]["sample_texts"]) < 5:
            if re.search(r'[a-zA-Z\u0600-\u06FF]', text):  # Latin or Arabic
...
def main() -> None:
    ...
    # Get audio clips (exclude macOS hidden files starting with ._)
    clip_wavs = [
    ...
    with log_path.open("w", encoding="utf-8") as log_f:
        tee_print(log_f, "=" * 60)
        tee_print(log_f, "TRANSCRIBE WITH SPEAKERS v5")
        tee_print(log_f, "=" * 60)
        ...
        tee_print(log_f, "PHASE 3: Transcription")
        tee_print(log_f, "-" * 40)
        ...
        for i, wav_path in enumerate(clip_wavs, 1):
            ...
            result = transcribe_audio(wav_path, whisper_model, log_f)
