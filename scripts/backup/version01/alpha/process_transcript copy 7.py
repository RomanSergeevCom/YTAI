...
Output:
    02_Transcripts/02_01_Runs/
        *_named.json      - JSON with speaker names and original IDs
        *_named.txt       - Human-readable transcript
        *_named.srt       - Full SRT with global timecodes
        *_speakers.txt    - Speaker info with context
        *_by_clips.xlsx   - Table: file | timecode | speaker | original_id | text
...
def extract_speaker_names_llm(segments: List[dict], speakers: Set[str], model: str, logger: logging.Logger, project_profile: str = "") -> Tuple[Dict[str, str], Dict[str, str]]:
    ...
    Args:
        project_profile: Optional text describing participants (from project_profile.txt)
    """
...
IMPORTANT: Use the names and roles from the profile above to match speakers!
Match speakers to known participants based on what they say and their role.
"""
...
1. If project profile is provided, MATCH speakers to known participants first
2. Look for self-introductions: "My name is...", "I am...", "I'm [Name]"
3. Look for how others address them: "Thank you Ahmed", "As Hadi mentioned"
4. The MOST ACTIVE speaker (most segments) is usually the main GUEST
5. The person ASKING QUESTIONS is the HOST/INTERVIEWER
6. If someone speaks Arabic, identify them by their ROLE (e.g. "Coffee Shop Owner")
7. Names like "Hadi", "Ahmed", "Roman" are real names - use them!
