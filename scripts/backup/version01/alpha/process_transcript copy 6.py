...
def extract_speaker_names_llm(segments: List[dict], speakers: Set[str], model: str, logger: logging.Logger) -> Tuple[Dict[str, str], Dict[str, str]]:
    """
    ...
    # Collect ALL lines for better analysis
    ...
    # Build comprehensive sample: 
    # - First 10 lines (introductions)
    # - Middle 10 lines (main content)  
    # - Last 5 lines (conclusions)
    # Up to 500 chars per line for full context
    transcript_parts = []
    
    # Sort speakers by segment count (most active first - likely main guest)
    ...
    for speaker in sorted_speakers:
        ...
        if not all_lines:
            ...
            all_lines = [seg.get("text", "") for seg in segments 
                        if seg.get("speaker") == speaker and seg.get("text", "").strip()][:10]
        
        # Take first 10 + middle 10 + last 5 for comprehensive view
        sample_lines = []
        
        # First 10 (introductions, who they are)
        sample_lines.extend(all_lines[:10])
        
        # Middle 10 (main content)
        if len(all_lines) > 25:
            mid = len(all_lines) // 2
            sample_lines.extend(all_lines[mid-5:mid+5])
        
        # Last 5 (conclusions, summaries)
        if len(all_lines) > 15:
            sample_lines.extend(all_lines[-5:])
        ...
        if unique_lines:
            transcript_parts.append(f"\n{speaker} ({count} segments - {'MOST ACTIVE' if count == max(speaker_counts.values()) else 'active' if count > 50 else 'minor'}):")
            for line in unique_lines:
                transcript_parts.append(f'  "{line[:500]}"')
    
    transcript = "\n".join(transcript_parts)
    speakers_list = ", ".join(sorted(speakers))
    
    # Add segment count info
    count_info = ", ".join([f"{s}: {speaker_counts[s]} segments" for s in sorted_speakers])
    
    prompt = f"""Analyze this interview transcript and identify WHO each speaker is.

This is a YouTube interview about business in Saudi Arabia / UAE.

SEGMENT COUNTS (helps identify roles):
{count_info}

KEY INSIGHT: The speaker with MOST segments is likely the MAIN GUEST being interviewed.
The speaker asking questions (often shorter segments) is the INTERVIEWER/HOST.

SPEAKERS TO IDENTIFY: {speakers_list}

TRANSCRIPT SAMPLES (sorted by activity level):
{transcript}

IDENTIFICATION RULES:
1. Look for self-introductions: "My name is...", "I am...", "I'm [Name]"
2. Look for how others address them: "Thank you Ahmed", "As Hadi mentioned"
3. The MOST ACTIVE speaker (most segments) is usually the main GUEST
4. The person ASKING QUESTIONS is the HOST/INTERVIEWER
5. If someone speaks Arabic, identify them by their ROLE (e.g. "Coffee Shop Owner")
6. Names like "Hadi", "Ahmed", "Roman" are real names - use them!

ROLE HIERARCHY:
- "Host" or "Interviewer" - person asking questions (usually fewer segments)
- "[Name]" - if you find their actual name, USE IT (e.g. "Hadi Dawani", "Roman")  
- "[Business Type] Owner" - for business owners (e.g. "Coffee Shop Owner")
- "Guest" - only if no better identifier found

CRITICAL REQUIREMENTS:
- The speaker with {max(speaker_counts.values())} segments is the MAIN GUEST - give them a proper name/title
- Each label must be UNIQUE
- NEVER leave any speaker as SPEAKER_XX
- Provide context describing what each person talks about

OUTPUT FORMAT - JSON only, no explanation:
{{
  "names": {{"SPEAKER_00": "Name or Role", "SPEAKER_01": "Name or Role"}},
  "context": {{"SPEAKER_00": "Who they are and main topics they discuss", "SPEAKER_01": "..."}}
}}

Identify ALL {len(speakers)} speakers with UNIQUE names/roles."""
...
RULES:
1. Compare speech patterns, vocabulary, and topics discussed
2. The HOST/INTERVIEWER asks questions - likely ONE person
3. The GUEST answers questions - likely ONE person  
4. If two speakers discuss the same topic from the same perspective, they might be the same person
5. Different languages (English vs Arabic) usually means DIFFERENT people

OUTPUT FORMAT - JSON only:
{{
  "merge_groups": [
    ["SPEAKER_00", "SPEAKER_02"],  // These are the same person
    ["SPEAKER_01", "SPEAKER_05"]   // These are the same person
  ],
  "reasoning": "Brief explanation of why speakers should be merged"
}}

If NO speakers should be merged, return:
{{"merge_groups": [], "reasoning": "All speakers appear to be different people"}}
"""
    ...
    try:
        ...
        if json_match:
            ...
            if merge_groups:
                logger.info(f"LLM suggests merging: {merge_groups}")
            ...
            for group in merge_groups:
                if len(group) >= 2:
                    # Find the speaker with most segments as the "primary"
            ...
            # Map minor speakers to UNKNOWN or nearest major speaker
...
Examples:
    ...
    if not args.skip_llm and not speaker_map:
        speaker_map, speaker_context = extract_speaker_names_llm(segments, original_speakers, args.llm_model, logger)
