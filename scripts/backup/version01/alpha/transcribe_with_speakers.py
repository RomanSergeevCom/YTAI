...
For pyannote.audio, you need a Hugging Face token:
    ...
    2. Accept terms at: https://huggingface.co/pyannote/speaker-diarization-3.1
       OR for better results: https://huggingface.co/pyannote/speaker-diarization-community-1
    3. Accept terms at: https://huggingface.co/pyannote/segmentation-3.0
    4. Create token at: https://huggingface.co/settings/tokens
    5. Put token in /Users/romansergeev/YTAI/config/HuggingFace-yt-prod.conf

Usage:
    python transcribe_with_speakers.py <audio_file> [--model large]
    
Supports:
    - pyannote.audio 3.x (returns Annotation with itertracks)
...
def load_env():
    ...
    if not config_file.exists():
        ...
        print("  echo '# HuggingFace token' > /Users/romansergeev/YTAI/config/HuggingFace-yt-prod.conf")
        print("  echo 'HF_TOKEN=hf_your_token' >> /Users/romansergeev/YTAI/config/HuggingFace-yt-prod.conf")
        return
...
def perform_diarization(audio_path: str, num_speakers: int, logger: logging.Logger, use_community_model: bool = False) -> list:
    """
    Perform speaker diarization using pyannote.audio.
    
    Supports both pyannote.audio 3.x and 4.x APIs.
    
    Args:
        audio_path: Path to audio file
        num_speakers: Number of speakers (None for auto-detect)
        logger: Logger instance
        use_community_model: If True, use community-1 model (better but requires accepting new terms)
    
    Returns:
        List of diarization segments with start, end, speaker keys
    """
    ...
    if torch.backends.mps.is_available():
        ...
        logger.info("Using MPS (Apple Silicon) for diarization")
    ...
    # Load audio with soundfile to bypass torchcodec issues
    ...
    try:
        if num_speakers:
            diarization_output = pipeline(audio_input, num_speakers=num_speakers)
        else:
    ...
    # Convert to list of segments - handle both pyannote 3.x and 4.x APIs
    segments = []
    
    logger.debug(f"Diarization output type: {type(diarization_output)}")
    logger.debug(f"Diarization output attributes: {[a for a in dir(diarization_output) if not a.startswith('_')]}")
    
    # Method 1: pyannote.audio 4.x DiarizeOutput with speaker_diarization attribute
    if hasattr(diarization_output, 'speaker_diarization'):
        logger.info("Using pyannote.audio 4.x API (speaker_diarization)")
        try:
            # speaker_diarization is iterable of (Segment, speaker) tuples
            ...
            logger.info(f"Extracted {len(segments)} segments via speaker_diarization")
        except Exception as e:
            logger.warning(f"Failed to iterate speaker_diarization: {e}")
    
    # Method 2: pyannote.audio 3.x Annotation with itertracks
    if not segments and hasattr(diarization_output, 'itertracks'):
        logger.info("Using pyannote.audio 3.x API (itertracks)")
        try:
            ...
            logger.warning(f"Failed to iterate via itertracks: {e}")
    
    # Method 3: Try exclusive_speaker_diarization (pyannote 4.x alternative)
    if not segments and hasattr(diarization_output, 'exclusive_speaker_diarization'):
        logger.info("Using pyannote.audio 4.x API (exclusive_speaker_diarization)")
        try:
            ...
            logger.info(f"Extracted {len(segments)} segments via exclusive_speaker_diarization")
        except Exception as e:
            logger.warning(f"Failed to iterate exclusive_speaker_diarization: {e}")
    
    # Method 4: Fallback - try to serialize and parse
    if not segments and hasattr(diarization_output, 'serialize'):
        logger.info("Using serialize fallback")
        try:
            serialized = diarization_output.serialize()
            logger.debug(f"Serialized output keys: {serialized.keys() if isinstance(serialized, dict) else type(serialized)}")
            
            if isinstance(serialized, dict):
                # Try common keys
                for key in ['speaker_diarization', 'diarization', 'segments', 'turns']:
                    if key in serialized:
                        ...
                        if segments:
                            logger.info(f"Extracted {len(segments)} segments via serialize['{key}']")
                            break
        except Exception as e:
            logger.warning(f"Failed to use serialize fallback: {e}")
    
    if not segments:
        logger.error("Could not extract diarization segments from output")
        logger.error(f"Output type: {type(diarization_output)}")
        logger.error(f"Available attributes: {[a for a in dir(diarization_output) if not a.startswith('_')]}")
        
        # Last resort: try to print the object for debugging
        try:
            logger.error(f"Output repr: {repr(diarization_output)[:500]}")
        except:
            pass
...
def main():
    ...
    parser.add_argument(
        "--community",
        action="store_true",
        help="Use community-1 model (better quality, requires accepting terms on HuggingFace)"
    )
    ...
    logger.info(f"Diarization: {'community-1' if args.community else 'speaker-diarization-3.1'}")
    ...
    try:
        ...
        logger.debug(f"pyannote.audio version: {pyannote.audio.__version__}")
    except:
        pass
    ...
    diarization = perform_diarization(audio_path, args.num_speakers, logger, use_community_model=args.community)
