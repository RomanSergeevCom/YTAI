...
"""
    ...
    # Build speakers list with counts for better context
    speakers_list = "\n".join([f"- {s} ({speaker_counts.get(s, 0)} utterances)" 
                               for s in sorted_speakers])
    
    # Improved prompt with stricter format rules
    prompt = f"""Analyze this interview transcript and identify each speaker.

This is a YouTube interview about business in UAE/Saudi Arabia.
{profile_section}
SPEAKERS TO IDENTIFY (you MUST provide a name for EACH one):
{speakers_list}

{transcript}

IDENTIFICATION RULES:
1. Person ASKING questions = Host/Interviewer (usually "Roman" for this channel)
2. Person EXPLAINING their business/experience at length = Guest (the interview subject)
3. Look for names: "My name is...", "I am...", "Thank you [Name]", "So [Name]..."
4. The speaker with MOST utterances who answers questions = MAIN GUEST
5. Short responses like "Yeah", "Okay", "Right" = likely minor speakers or host backchanneling

NAME FORMAT RULES - CRITICAL:
- Use SIMPLE names only: "Roman", "Hadi", "Ahmed", "Host", "Guest"  
- NO special characters allowed: no slashes (/), no parentheses (), no brackets []
- NO descriptions in the name field - put descriptions in context field only
- If you found actual name, use it: "Hadi" not "Hadi the Business Owner"
- If no name found, use simple role: "Host", "Guest", "Assistant"
- Maximum 2 words per name

YOU MUST RETURN ALL {len(sorted_speakers)} SPEAKERS: {', '.join(sorted_speakers)}

OUTPUT FORMAT - JSON only, no markdown:
{{
  "names": {{
    "{sorted_speakers[0]}": "SimpleName",
    "{sorted_speakers[1] if len(sorted_speakers) > 1 else 'SPEAKER_01'}": "SimpleName"
  }},
  "context": {{
    "{sorted_speakers[0]}": "Description goes here",
    "{sorted_speakers[1] if len(sorted_speakers) > 1 else 'SPEAKER_01'}": "Description"
  }}
}}"""
    ...
    try:
        ...
        if json_match:
            ...
            if "names" in data:
                ...
                # НОВОЕ: Проверяем что LLM вернул всех спикеров
                missing = speakers - set(raw_names.keys())
                if missing:
                    logger.warning(f"LLM missed {len(missing)} speakers: {missing}")
                    # Автозаполнение пропущенных спикеров
                    for speaker in missing:
                        count = speaker_counts.get(speaker, 0)
                        if count > 100:
                            raw_names[speaker] = "Main Guest"
                            raw_context[speaker] = f"Primary interview subject ({count} utterances)"
                        elif count > 50:
                            raw_names[speaker] = "Guest"
                            raw_context[speaker] = f"Interview participant ({count} utterances)"
                        elif count > 20:
                            raw_names[speaker] = "Co-Host"
                            raw_context[speaker] = f"Secondary speaker ({count} utterances)"
                        else:
                            speaker_num = re.search(r'\d+', speaker)
                            num = int(speaker_num.group()) + 1 if speaker_num else 0
                            raw_names[speaker] = f"Speaker {num}"
                            raw_context[speaker] = f"Minor participant ({count} utterances)"
                        logger.info(f"Auto-filled {speaker} -> '{raw_names[speaker]}'")
...
def validate_speaker_names(raw_names: Dict[str, str], speakers: Set[str], logger: logging.Logger) -> Dict[str, str]:
    ...
    IMPROVED: Clean special chars instead of rejecting.
    """
    validated = {}
    used_names = set()
    ...
    for speaker in speakers:
        ...
        # НОВОЕ: Очистка спецсимволов ПЕРЕД валидацией
        # "Guest (Coffee Shop Owner)" -> "Guest Coffee Shop Owner"
        # "Moderator/Host" -> "Moderator Host"
        name = re.sub(r'[/\\()\[\]]', ' ', name)  # Заменяем /, \, (, ), [, ] на пробелы
        name = re.sub(r'\s+', ' ', name).strip()  # Убираем лишние пробелы
        
        # Если имя слишком длинное - берём первые 2-3 слова
        words = name.split()
        if len(words) > 3:
            name = ' '.join(words[:3])
            logger.debug(f"Truncated name for {speaker}: '{name}'")
        ...
        # Проверка на допустимые символы (после очистки должно работать)
        ...
        if is_valid:
            ...
            # УЛУЧШЕНО: Используем номер из SPEAKER_XX
            speaker_num = re.search(r'\d+', speaker)
            if speaker_num:
                fallback = f"Speaker {int(speaker_num.group()) + 1}"
            else:
...
def save_speakers_txt(speaker_map: Dict[str, str], speaker_context: Dict[str, str], 
    ...
    """
    Save speakers verification file with ID, name, context, and SAMPLE UTTERANCES.
    This helps manually verify that speaker names are correctly assigned.
    """
    # Collect data per original speaker ID
    speaker_data = {}
    
    for seg in segments:
        # Get original ID (before renaming)
    ...
    lines = [
        ...
        "=" * 70,
        "",
        "Review the sample utterances below to verify speaker identification.",
        "If names are wrong, edit speaker_names.json and re-run with --skip-llm",
        "",
        "QUICK CHECK:",
        "- Person ASKING questions = Host/Interviewer (Roman)",
        "- Person ANSWERING about their business = Guest (Hadi)",
        "",
        "=" * 70,
        "",
    ]
    ...
    for original_id in sorted_speakers:
        ...
        lines.append("")
        lines.append("SAMPLE UTTERANCES (verify this matches the assigned name):")
        lines.append("")
        
        for i, utt in enumerate(data["utterances"][:5], 1):  # Show first 5
            time_str = format_timestamp(utt["time"])
            lines.append(f"  [{time_str}] \"{utt['text']}\"")
        
        if len(data["utterances"]) > 5:
            lines.append(f"  ... and {data['count'] - 5} more utterances")
        
        lines.append("")
        
        # Add verification hint based on content
        sample_texts = " ".join([u["text"].lower() for u in data["utterances"][:5]])
        
        hints = []
        if any(q in sample_texts for q in ["?", "could you", "what do you", "tell us", "how do you"]):
            hints.append("⚠️  Contains QUESTIONS - likely INTERVIEWER/HOST")
        if any(w in sample_texts for w in ["my business", "i started", "we opened", "our company", "i own"]):
            hints.append("⚠️  Talks about OWN BUSINESS - likely GUEST")
        if data["count"] > 200:
            hints.append(f"⚠️  MOST ACTIVE speaker ({data['count']} segments) - usually MAIN GUEST")
        if data["count"] < 50 and data["count"] > 20:
            hints.append("⚠️  Moderate activity - could be HOST or secondary speaker")
    ...
    # Add manual fix instructions at the end
    lines.extend([
        "=" * 70,
        "HOW TO FIX WRONG NAMES:",
        "=" * 70,
        "",
        "1. Edit speaker_names.json:",
        "   {",
        '     "SPEAKER_00": "Roman",      <- Change to correct name',
        '     "SPEAKER_05": "Hadi Dawani" <- Change to correct name', 
        "   }",
        "",
        "2. Re-run script with --skip-llm to use your manual names:",
        "   python process_transcript.py --project \"...\" --skip-llm",
        "",
        "Or delete speaker_names.json and re-run to try LLM again.",
        "",
    ])
...
def main():
    ...
    parser.add_argument("--force-llm", action="store_true",
                        help="Force LLM speaker naming (ignore cached speaker_names.json)")
    ...
    # Try manual map first (unless --force-llm)
    if args.force_llm:
        logger.info("Force LLM mode - ignoring cached speaker_names.json")
    else:
