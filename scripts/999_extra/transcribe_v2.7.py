...
# Based on real-world benchmarks: Whisper ~1.9x, pyannote ~3-5x, combined ~1.2-1.5x
ESTIMATE_SPEED = {
    "tiny":     8.0,
    "base":     6.0,
    "small":    4.0,
...
def estimate_processing_time(total_audio_seconds: float, model_size: str, num_files: int) -> float:
    ...
    """
    speed = ESTIMATE_SPEED.get(model_size, 1.5)
    processing_time = total_audio_seconds / speed
    
    # Add model loading overhead (first file only, ~30s Whisper + ~15s pyannote)
    overhead = 45.0
    
    # Add per-file overhead (ffmpeg, file I/O, ~5s each)
    per_file_overhead = num_files * 5.0
