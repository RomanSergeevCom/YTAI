...
Создавать из Trello-шаблона **«YT Template — шаблон доски канала»**
(id `66fc09fff3a55640376df3db`, isTemplate=true, бывший «YT Demo») — в нём уже
11 списков, метка Blocked и карточка-инструкция «Это название папки в гугл диске»
в 01Created. После создания: имя доски → «YTXX — Название», добавить канал в
`BOARD_NAMES` здесь и (опционально) цвет в FALLBACK_COLORS/`channels.json`.
Кокпит подхватит доску автоматически (`BOARD_RE ^YT[A-Z]`); сам шаблон в кокпит
не попадает (имя «YT␣…» не матчится).

Скрипт идемпотентен (уже-нумерованные списки не трогает, только порядок).
