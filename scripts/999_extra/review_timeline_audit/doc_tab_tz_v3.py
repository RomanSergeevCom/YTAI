...
WIDTHS = [24, 54, 22, 300, 276]          # v7: колонке «Материал» шире (сумма та же 676pt)
FONT = 9
...
CHAPTERS = [
    ...
    {'no': 7, 'sec': 1558, 'name': '➕ РЕКОРДЫ АУКЦИОНОВ — НОВАЯ', 'img': 'mock_card_auctions.png', 'color': 'Yellow'},
...
def build_head(act, rejected):
    ...
    head = [
        ...
        (0, 'Как читать: каждый таймкод — отдельной строкой («таймкод ▸ что там»), как в карте структуры · '
            'справа — крупное превью: наш драфт или стрелка наложены на реальный кадр и кропнуты на то, о чём '
            'речь · в опечатках «было → стало» изменённые буквы — красным · суммы — цифрами.', {}),
        (0, 'Рабочая секвенция: YTUVI01_5_Review_v6_tz_v{N} (панель UXP → Review → Review_v6). '
            ...
            'V4 = плашки ТЗ (полный текст + ссылки: маркер клипа / кнопка панели '
            '«Copy ТЗ @ playhead») · V5 = стрелки правок на кадре · V6 = прозрачные главы + подглавы + '
            'прогресс перечислений гл.08/09. Маркеры секвенции = только 10 разноцветных глав. '
            'Глава «Проверка геммолога» удалена (07.09); CTA сразу после конца рендера.', {'bold': True}),
        (0, f'Живой чек-лист со статусами — лист «ТЗ монтажёру» в таблице Review Notes: {SHEET_URL} · '
            'Полный разбор с транскрибацией — вкладка «Ревью v2 · правки».', {}),
        (0, f'📁 Все материалы (на каждом файле — коммент с ТЗ, таймкодом и источником): '
            f'{MATERIALS_FOLDER} · 📁 Папка проекта: {PROJECT_FOLDER} · '
            f'📁 Спринт YTUVI S1: {SPRINT_FOLDER}', {}),
        (2, f'❓ Решения Романа ({len(decisions)})', {}),
    ]
    if rejected:
        head.append((0, '🚫 Снято Романом (09–10.09, не менять, номера сохранены): '
                     + ', '.join(f"{p['num']} {clean(p['title'])[:40]}" for p in rejected), {}))
    for p in decisions:
        head.append((0, f"• {p['num']} · {p['v1_tc']} · {clean(p['title'])} — {p['decision']}", {}))
    return head
