...
CTL="$HOME/.cache/$PROJ"
PY_VLM="${PY_VLM:-$HOME/YTAI/environment/.venv_llm/bin/python}"
mkdir -p "$CTL"

stage="${1:-}"; cmd="${2:-}"
case "$stage" in
  vlm) SCRIPT=s3_vlm.py;        OUTF="$W6/vlm_v6.jsonl" ;;
  llm) SCRIPT=s4_llm.py;        OUTF="$W6/llm_v6.json"  ;;
  ocr) SCRIPT=s2_ocr_hires.py;  OUTF="$W6/ocr_hires.jsonl" ;;
...
case "$cmd" in
  start)
    ...
    ( cd "$W6" && nohup caffeinate -dims "$PY_VLM" -u "$SCRIPT" >> "$LOG" 2>&1 < /dev/null & echo $! > "$PID" )
    sleep 1
