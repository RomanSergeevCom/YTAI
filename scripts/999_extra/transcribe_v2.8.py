...
def progress_bar(
    ...
    """
    Render a colored progress bar with stats.
    
    Example:
      [████████████ 42% ░░░░░░░░░░░░░░░] 3/6 folders │ 1h 42m / ~3h 20m │ 0.8x RT
    """
    ...
    # Build bar with percentage in the middle
    pct_str = f"{int(pct * 100)}%"
    bar_chars_filled = "█" * filled
    bar_chars_empty = "░" * empty
    
    # Insert percentage into the bar
    full_bar = bar_chars_filled + bar_chars_empty
    mid = width // 2 - len(pct_str) // 2
    if mid >= 0 and mid + len(pct_str) <= width:
        bar_list = list(full_bar)
        for i, ch in enumerate(pct_str):
            bar_list[mid + i] = ch
        full_bar = "".join(bar_list)
        # Color: filled part in color, percentage always white, empty in gray
        bar_colored = (
        ...
        bar_colored = f"{color}{bar_chars_filled}{C.RESET}{C.GRAY}{bar_chars_empty}{C.RESET}"
    ...
    return f"[{bar_colored}] {counter} │ {time_str}{speed_str}"
...
def find_batch_folders(parent: Path) -> List[Tuple[Path, List[Path]]]:
    """
    Find all subfolders that contain media files.
    Returns list of (folder_path, media_files) tuples, sorted by folder name.
    Skips hidden folders and folders with no media.
    """
    results = []
    for item in sorted(parent.iterdir(), key=lambda p: natural_sort_key(p.name)):
        if item.is_dir() and not item.name.startswith("."):
            media = find_media_in_folder(item)
            if media:
    ...
    if args.batch:
        ...
        for idx, bp in enumerate(batch_plan, 1):
            ...
            try:
                ...
                speed_x = processed_audio_so_far / batch_elapsed if batch_elapsed > 0 else 0
                # Recalculate total estimate based on actual speed
                if processed_audio_so_far > 0:
                    actual_rate = batch_elapsed / processed_audio_so_far
                    remaining_audio = sum(bp2["audio_duration"] for bp2 in batch_plan[idx:])
                    est_total = batch_elapsed + remaining_audio * actual_rate
                else:
                    est_total = total_batch_est
                print("")
                print(progress_bar(idx, len(batch_plan), batch_elapsed, est_total, "folders", speed_x=speed_x))
    ...
    for i, video_path in enumerate(videos, 1):
        ...
        try:
            ...
            if len(videos) > 1:
                ...
                speed_x = processed_audio / folder_elapsed if folder_elapsed > 0 else 0
                if processed_audio > 0:
                    actual_rate = folder_elapsed / processed_audio
                    remaining_audio = sum(video_durations[v] for v in videos[i:])
                    est_total_recalc = folder_elapsed + remaining_audio * actual_rate
                else:
                    est_total_recalc = est_seconds
                print("")
                print(progress_bar(i, len(videos), folder_elapsed, est_total_recalc, "files", speed_x=speed_x))
