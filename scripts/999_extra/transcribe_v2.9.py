...
Features:
  ...
  - Time estimation with per-file ETA
...
def process_video(
    ...
    audio_path = result_dir / f"{video_name}_audio.wav"
    output_base = result_dir / f"{video_name}_transcript"
  ...
  2. Accept licenses:
    ...
    if args.batch:
        ...
        for idx, bp in enumerate(batch_plan, 1):
            ...
            try:
                # Set up output directory for this folder
                proj_name = resolve_project_name(folder, None)
                out_dir = folder / f"{proj_name}_transcription"
    ...
    # Save session log
    if all_results:
        log_dir = output_dir if output_dir else (all_results[0]["metadata"].get("source_path", "") and Path(all_results[0]["metadata"]["source_path"]).parent)
        if log_dir:
            log_ts = datetime.now().strftime("%Y%m%d_%H%M%S")
