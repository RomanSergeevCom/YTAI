...
def cam_of(clip, source):
    """Имя камеры — папка `CAM-*` между сценой и файлом ('' если её нет).

    ⚠️ Раньше здесь был просто `parts[1]` — папка СРАЗУ под сценой. Это верно,
    пока дерево ровно `{сцена}/{камера}/файл`, и ломается, как только между ними
    появляются свои уровни. На YTUVIE раскладка повторяет структуру курса
    (`{блок}/{тема}/{урок}/{камера}/файл`, решение Романа 23.09.2026), и `parts[1]`
    вернул бы имя ТЕМЫ. Камерой это не является, но и ошибкой не выглядит: профиль
    канала молча разветвился бы на «камеры» вида `1.1_Karta_tseha`, проявка каждой
    из них оказалась бы не выбрана, и раскладка отказала бы на всём дне.

    Поэтому камеру ищем по её собственному признаку — префиксу `CAM-`, — а не по
    месту в пути. Прежнее поведение сохранено запасной веткой: дерево без `CAM-*`
    по-прежнему отдаёт папку под сценой.
    """
    parts = clip.relative_to(source).parts       # (сцена, …, CAM-A_FX3, файл.MP4)
    for p in parts[1:-1]:
        if p.startswith("CAM-"):
            return p
