...
def _load_tx_lookup(project: Path, scene_name: str) -> dict:
    """Load premiere_transcript paths from the transcription ingest for a scene.

    Reads Video/{scene}/{scene}_ingest.json (generated by ingest_json.py after
    transcription) and returns {clip_id: absolute_premiere_transcript_path}.
    """
    tx_ingest = project / "01_Media" / "Source" / "Video" / scene_name / f"{scene_name}_ingest.json"
    if not tx_ingest.exists():
        return {}
    with open(tx_ingest) as f:
        data = json.load(f)
    return {
        c["clip_id"]: c["premiere_transcript"]
        for c in data.get("clips", [])
        if c.get("premiere_transcript")
    }
...
def convert_scene(scene_data: dict, project: Path,
    ...
    clips_out = []
    tx_lookup = _load_tx_lookup(project, scene_name)
    ...
    for cr in scene_data["clips"]:
        ...
        if clip_id in tx_lookup:
            clip_entry["premiere_transcript"] = tx_lookup[clip_id]
        if dji_audio:
...
def generate_uxp_ingest(
    ...
    # Primary: Setup/Ingest/  |  Fallback: Setup/ (backward compat)
    ingest_subdir = setup_dir / "Ingest"
    skip_stems = {f"{code}_ingest", f"{project.name}_ingest"}

    if ingest_subdir.exists():
        scene_files = sorted(ingest_subdir.glob("*_ingest.json"))
    else:
        scene_files = sorted(
    ...
    if not scene_files:
        print(f"ERROR: No per-scene ingest.json files found in {ingest_subdir} or {setup_dir}", file=sys.stderr)
        sys.exit(1)
    ...
    # ── Assemble output ───────────────────────────────────────────────────────
    # Populate per-scene SRT paths for transcript import
    scene_captions = {}
    video_dir = project / "01_Media" / "Source" / "Video"
    for sn in sorted(set(c["scene"] for c in all_clips)):
        cap_srt = video_dir / sn / f"{sn}_1_Ingest_captions.srt"
        if cap_srt.exists():
            scene_captions[sn] = str(cap_srt)

    n_tx = sum(1 for c in all_clips if c.get("premiere_transcript"))
    ...
    uxp_ingest = {
        ...
        "files": {
            ...
            "scene_captions_srt": scene_captions or None,
        },
    }

    total_dji = sum(len(c.get("dji_audio", [])) for c in all_clips)
    print(f"\nTotal   : {len(all_clips)} clips  {total_dji} DJI WAVs  {n_tx} transcripts")
