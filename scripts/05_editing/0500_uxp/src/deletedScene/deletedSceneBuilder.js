/**
 * deletedSceneBuilder.js — Build per-scene Deleted Scene sequences.
 *
 * For each scene, creates a Deleted Scene sequence that mirrors the Ingest timeline
 * with Assembly-used segments removed (gaps). Editor sees full footage
 * minus what's already in Assembly.
 *
 * Per-scene sequences:
 *   {CODE}_3_DeletedScene_01_apartment
 *   {CODE}_3_DeletedScene_02_outdoor
 *
 * Color scheme:
...
/**
 * Determine deleted scene category for a segment.
 */
function getDeletedSceneCategory(seg) {
...
/**
 * Determine label color index for a deleted scene segment.
 */
function getDeletedSceneColorIdx(seg, opts) {
  if (opts && opts.producerSpeaker && seg.speaker === opts.producerSpeaker) {
    return DELETED_SCENE_PRODUCER_COLOR.labelIdx;
  }
  if (seg.speaker && seg.speaker !== '') {
    return DELETED_SCENE_EXPERT_COLOR.labelIdx;
  }
  if (seg.block === 99) return DELETED_SCENE_COLOR_MAP.cut.labelIdx;
  ...
  var cat = getDeletedSceneCategory(seg);
  return DELETED_SCENE_COLOR_MAP[cat].labelIdx;
}
...
/**
 * Build deleted scene segments for a single scene (complement approach).
 *
 ...
 * @returns {Array} Deleted scene segments sorted by sourceFile + inSec
 */
function buildSceneDeletedSceneSegments(allSegments, sceneClipNames, clipDurations) {
  ...
  // 2. Group brief deleted scene segments (use=FALSE or block=99) by clip
  var briefByClip = {};
  ...
  // 3. For each clip, compute complement and build deleted scene segments
  var allDeletedSceneSegs = [];
  ...
  for (var ci = 0; ci < sceneClipNames.length; ci++) {
    ...
    for (var ri = 0; ri < complementRanges.length; ri++) {
      ...
      for (var bri = 0; bri < briefInRange.length; bri++) {
        ...
        if (bs.duration >= MIN_GAP_DURATION) allDeletedSceneSegs.push(bs);
      }
      ...
      for (var gi = 0; gi < sortedBrief.length; gi++) {
        ...
        if (bIn > pos && bIn - pos >= MIN_GAP_DURATION) {
          allDeletedSceneSegs.push(createGapSegment(clipName, pos, bIn));
        }
        pos = Math.max(pos, Math.min(bSeg.outSec, range.out));
      }
      if (range.out > pos && range.out - pos >= MIN_GAP_DURATION) {
        allDeletedSceneSegs.push(createGapSegment(clipName, pos, range.out));
      }
    }
  ...
  allDeletedSceneSegs.sort(function (a, b) {
    if (a.sourceFile !== b.sourceFile) return a.sourceFile < b.sourceFile ? -1 : 1;
    return a.inSec - b.inSec;
  });

  return { segments: allDeletedSceneSegs, assemblyGapMarkers: assemblyGapMarkers };
}
...
/**
 * Build a single Deleted Scene sequence for one scene.
 *
 * @param {Object} project - Premiere project
 * @param {Object} clipMap - { filename: projectItem }
 * @param {Array} deletedSceneSegs - Deleted scene segments for this scene
 ...
 */
async function buildSingleDeletedSceneSequence(project, clipMap, deletedSceneSegs, clipOffsets, seqName, logger, opts) {
  if (deletedSceneSegs.length === 0) {
    if (logger) logger.info('No deleted scene segments for ' + seqName + ' — skipping');
    return { sequence: null, clipCount: 0, totalDuration: 0 };
  }

  // Calculate absolute timeline positions
  for (var pi = 0; pi < deletedSceneSegs.length; pi++) {
    var s = deletedSceneSegs[pi];
    s._timelinePosition = (clipOffsets[s.sourceFile] || 0) + s.inSec;
  }
  deletedSceneSegs.sort(function (a, b) { return a._timelinePosition - b._timelinePosition; });

  // Clean existing
  await cleanExistingSequence(project, seqName, logger);

  // Create sequence from first clip
  var firstSeg = deletedSceneSegs[0];
  ...
  var firstColorIdx = getDeletedSceneColorIdx(firstSeg, opts);
  ...
  if (!seq) throw new Error('Failed to create Deleted Scene sequence: ' + seqName);
  ...
  if (logger) {
    var cat = getDeletedSceneCategory(firstSeg);
    logger.info('[1/' + deletedSceneSegs.length + '] ' + firstSeg.id + ': ' + firstSeg.sourceFile +
  ...
  for (var i = 1; i < deletedSceneSegs.length; i++) {
    var seg = deletedSceneSegs[i];
    ...
    var colorIdx = getDeletedSceneColorIdx(seg, opts);
    ...
    if (logger) {
      var segCat = getDeletedSceneCategory(seg);
      logger.info('[' + (i + 1) + '/' + deletedSceneSegs.length + '] ' + seg.id + ': ' + seg.sourceFile +
...
/**
 * Build per-scene Deleted Scene sequences.
 *
 * Creates one Deleted Scene sequence per scene:
 *   {CODE}_3_DeletedScene_{scene}  (e.g. YTXX01_3_DeletedScene_01_apartment)
 *
 ...
 * @param {Object} sceneMap - { sceneName: [clipFilename, ...] } — if null, builds single DeletedScene
 * @returns {{ sequences: Array, totalClipCount, assemblyGapMarkers }}
 */
async function buildDeletedSceneSequence(project, clipMap, segments, projectCode, logger, clipDurations, opts, briefVersion, sceneMap) {
  // If no scene map, try to derive from segments or build single DeletedScene (backward compat)
  ...
  for (var si = 0; si < sceneNames.length; si++) {
    ...
    // Sequence name: YTXX01_3_DeletedScene_01_apartment_v1
    var seqName = sceneName === 'all'
      ? projectCode + '_3_DeletedScene' + vSuffix
      : projectCode + '_3_DeletedScene_' + sceneName + vSuffix;

    if (logger) logger.info('=== Scene: ' + sceneName + ' (' + sceneClips.length + ' clips) → ' + seqName + ' ===');

    // Build deleted scene segments for this scene
    var sceneResult = buildSceneDeletedSceneSegments(segments, sceneClips, clipDurations);
    var deletedSceneSegs = sceneResult.segments;
    allGapMarkers = allGapMarkers.concat(sceneResult.assemblyGapMarkers);

    if (logger) {
      var gapCount = deletedSceneSegs.filter(function(s) { return s._isGap; }).length;
      var briefCount = deletedSceneSegs.length - gapCount;
      logger.info('Deleted scene segments: ' + briefCount + ' from brief + ' + gapCount + ' gaps = ' + deletedSceneSegs.length + ' total');
    }
    ...
    // Build the sequence
    var buildResult = await buildSingleDeletedSceneSequence(
      project, clipMap, deletedSceneSegs, offsetInfo.offsets, seqName, logger, opts
    );

    buildResult.sceneName = sceneName;
    buildResult.totalDuration = offsetInfo.totalDuration;
    buildResult.segments = deletedSceneSegs;
...
module.exports = {
  buildDeletedSceneSequence,
  buildSceneDeletedSceneSegments,
  buildSingleDeletedSceneSequence,
