...
async function validateScreensBuild(sequence, screenResult) {
  ...
  try {
    ...
    if (!items) items = [];
    if (items.length === screenResult.clips) ok('V1 Assembly: ' + items.length + '/' + expectedV1 + ' segments');
    else warn('V1 Assembly: ' + items.length + '/' + expectedV1 + ' segments');
...
/**
 * Debug Screen Cues (v1.9.3) — developer convenience:
 ...
 * Needs: brief + imported clips (00_Source) + optional PNGs (screen_cues/).
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ingestLogger.info('Version 1.9.3');
  assemblyLogger.info('0500_uxp initialized');
  assemblyLogger.info('Version 1.9.3');
  reviewLogger.info('0500_uxp initialized');
  reviewLogger.info('Version 1.9.3');
  screensLogger.info('0500_uxp initialized');
  screensLogger.info('Version 1.9.3');
});
