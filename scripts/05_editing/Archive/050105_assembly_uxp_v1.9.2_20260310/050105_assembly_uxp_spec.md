# 050105_assembly_uxp — Specification v1.9.0
...
- SCREEN CUES: `{project}_edit_brief.json` → `screens[]` массив (опционально)

**Выход (Premiere Pro):**
- INGEST: бины `00_Source/`, `01_Sequence/`, `02_Transcripts/`, секвенция `{project}_1_Ingest`
- ASSEMBLY: секвенция `{project}_2_Assembly` (V1: trimmed, colored clips + markers; V2: screen cues Orange)
...
└── src/screens/
    ...
    └── screenBuilder.js  ← buildScreenCues(), buildSegmentPositionMap(), getScreenTimelinePosition()
```
...
4.5. **Screen Cues** *(non-fatal, если screens[] есть)* — V2 клипы Orange + Comment маркеры Orange
...
6.5. **Screen Cues SRT** *(non-fatal, если screens[] есть)* — import `{project}_4_ScreenCues_captions.srt`

### Screen Cues Pipeline (Step 4.5)

- **Вход:** `screens[]` из edit_brief.json (опционально)
- **V2 клипы:** тот же source clip что у привязанного сегмента, overwrite на V2 (videoTrackIndex=1)
  - Trimmed: `screen.tc_in` → `tc_in + 2s`, Orange color label
  - Имя: `[SCR] chapter_title`, `[SCR] lower_third` и т.д.
- **Comment маркеры:** Orange, позиция = seg._timelineStart + offset
  - Comment = `[TYPE] title | subtitle | body` (truncated 200 chars)
- **7 типов:** chapter_title, half_overlay, three_fifths_overlay, lower_third, product_shot, list, info_graphic
- **Модули:** `src/screens/screenParser.js`, `src/screens/screenBuilder.js`
- **Non-fatal:** try/catch, log warning, Assembly продолжает без Screen Cues
- **Backward compatible:** brief без screens[] → steps 4.5 и 6.5 пропускаются
...
```json
{
  ...
  "screens": [{
    "screen_id": "scr_001", "type": "chapter_title",
...
│
├─ Step 4.5: buildScreenCues() [screenBuilder.js] (non-fatal, if screens[] exists)
│  ├─ buildSegmentPositionMap()          ← { segId: timelineStartSec }
│  ├─ FOR EACH screen:
│  │   ├─ getScreenTimelinePosition()    ← segStart + offset
│  │   ├─ applyColorToItem(Orange)       ← цвет ПЕРЕД overwrite
│  │   ├─ setSourceInOut(tcIn, tcIn+2s)  ← обрезка для V2 клипа
│  │   ├─ createOverwriteItemAction(V2)  ← videoTrackIndex=1
│  │   └─ clearSourceInOut()
│  ├─ TRANSACTION 1: createAddMarkerAction × N (Comment type)
│  ├─ markersOwner.getMarkers() → filter [SCR] markers
│  └─ TRANSACTION 2: createSetColorByIndexAction(Orange) × N
│
...
│
└─ Step 6.5: importCaptionsSrt() (non-fatal, if screens[] exists)
   └─ Ищет {project}_4_ScreenCues_captions.srt рядом с brief
```
...
├── ЧИТАЕТ: src/screens/screenParser      ← parseScreens()
└── ЧИТАЕТ: src/screens/screenBuilder     ← buildScreenCues()
...
└── screens/
    ...
    └── screenBuilder.test.js  ← buildSegmentPositionMap, getScreenTimelinePosition, buildScreenCues (15 тестов)
```
