...
describe('parseScreens', () => {
  it('parses valid screens correctly', () => {
    ...
    var raw = [
      { screen_id: 'scr_001', type: 'chapter_title', segment_id: 'seg_001', tc_in: '00:00.0', title: 'Introduction' },
    ...
    assert.equal(result.screens[0].type, 'chapter_title');
  ...
  it('skips screens with missing segment_id', () => {
    var segs = makeSegments();
    var raw = [
      { screen_id: 'scr_001', type: 'chapter_title', segment_id: 'seg_999', title: 'Missing' }
    ];
    var result = parseScreens(raw, segs);
    assert.equal(result.screens.length, 0);
  ...
  it('skips screens with missing required fields', () => {
    var segs = makeSegments();
    var raw = [
      { screen_id: 'scr_001', type: 'list', segment_id: 'seg_001', title: 'My List' }
      // 'list' requires title AND body — body is missing
    ];
    var result = parseScreens(raw, segs);
    assert.equal(result.screens.length, 0);
    assert.ok(result.warnings[0].includes('missing required fields'));
    assert.ok(result.warnings[0].includes('body'));
  });
  ...
  it('truncates long title/subtitle/body', () => {
    ...
    var raw = [
      { screen_id: 'scr_001', type: 'half_overlay', segment_id: 'seg_001', title: longTitle, body: longBody }
    ];
    var result = parseScreens(raw, segs);
  ...
  it('auto-generates screen_id if missing', () => {
    var segs = makeSegments();
    var raw = [
      { type: 'chapter_title', segment_id: 'seg_001', title: 'No ID' }
    ];
    var result = parseScreens(raw, segs);
    assert.equal(result.screens.length, 1);
    assert.equal(result.screens[0].id, 'scr_001');
  ...
  it('normalizes type to lowercase', () => {
    var segs = makeSegments();
    var raw = [
      { screen_id: 'scr_001', type: 'Chapter_Title', segment_id: 'seg_001', title: 'Hello' }
    ];
    var result = parseScreens(raw, segs);
    assert.equal(result.screens[0].type, 'chapter_title');
  });
  ...
  it('can reference use=FALSE segments', () => {
    var segs = makeSegments();
    var raw = [
      { screen_id: 'scr_001', type: 'chapter_title', segment_id: 'seg_004', title: 'From cut segment' }
    ];
    var result = parseScreens(raw, segs);
    assert.equal(result.screens.length, 1);
...
describe('formatMarkerComment', () => {
  it('formats type + title', () => {
    var screen = { type: 'chapter_title', title: 'Introduction', subtitle: '', body: '' };
    var result = formatMarkerComment(screen);
    assert.ok(result.includes('[CHAPTER TITLE]'));
  ...
  it('replaces newlines in body with slashes', () => {
    var screen = { type: 'list', title: 'Items', subtitle: '', body: 'Line1\nLine2\nLine3' };
    var result = formatMarkerComment(screen);
    assert.ok(result.includes('Line1 / Line2 / Line3'));
  });

  it('truncates to MAX_MARKER_COMMENT chars', () => {
    var screen = { type: 'info_graphic', title: 'T'.repeat(100), subtitle: 'S'.repeat(50), body: 'B'.repeat(100) };
    var result = formatMarkerComment(screen);
...
describe('formatSrtContent', () => {
  it('formats multi-line SRT text', () => {
    var screen = { type: 'half_overlay', title: 'Key Numbers', subtitle: '', body: 'Growth 40%\n150+ clients' };
    var result = formatSrtContent(screen);
    assert.ok(result.includes('[HALF OVERLAY]'));
