 ...
 *
 * Two independent pipelines in one panel:
 ...
 *
 * INGEST and ASSEMBLY modules do NOT import each other.
 * The only connection is the 00_Source bin (created by INGEST, read by ASSEMBLY).
 */
...
let assemblyState = { data: null, segments: [], blocks: [], projectName: '', filePath: null, building: false, clipMap: null };
...
function loadBriefFromString(jsonString, filePath) {
  ...
  $('btn-apply-colors').removeAttribute('disabled');
...
/**
 * Apply Colors button handler (Variant A):
 * 1. Applies color labels to ProjectItems via clipMap
 * 2. Rebuilds the Assembly sequence so clips inherit updated colors
 *
 * Adobe UXP limitation: existing timeline clips do NOT update when ProjectItem
 * color changes. A full rebuild (buildAssembly) is required to see new colors.
 */
async function applyColorsOnly() {
  if (assemblyState.segments.length === 0) {
    assemblyLogger.error('No brief loaded — load Edit Brief first');
    return;
  }

  assemblyLogger.info('=== APPLY COLORS (with rebuild) ===');

  // If we have clipMap from a previous build, apply colors first for diagnostic logging
  if (assemblyState.clipMap && Object.keys(assemblyState.clipMap).length > 0) {
    try {
      const project = await ppro.Project.getActiveProject();
      if (project) {
        const useSegs = assemblyState.segments.filter(s => s.use && s.block !== 99);
        await applyAssemblyColors(project, assemblyState.clipMap, useSegs, assemblyLogger);
      }
    } catch (e) {
      assemblyLogger.debug('Pre-rebuild color apply: ' + e.message);
    }
  }

  // Rebuild assembly — buildAssembly includes Step 2.5 (colors) before sequence creation
  assemblyLogger.info('Rebuilding assembly to apply colors on timeline...');
  await buildAssembly();

  assemblyLogger.info('=== APPLY COLORS COMPLETE ===');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('btn-apply-colors').addEventListener('click', applyColorsOnly);
