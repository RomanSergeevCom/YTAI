#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Google Doc «Монтажный план v2» (pageless) из doc_content_v2.json.

rows:
  {'kind':'act','text'}                       — секция-акт (тёмная заливка, 12pt bold)
  {'kind':'chapter','text'}                   — глава (голубая заливка, bold)
  {'kind':'row','cells':[...6...],'img':path,'shade':...}
Картинка вставляется в ячейку «экран» (index 4) НАД подписью; thumbnailLink
из манифеста frames_manifest.json ({basename: uri}).
"""
...
CONTENT = sys.argv[1] if len(sys.argv) > 1 else os.path.join(WORK, 'doc_content_v2.json')
        ...
        for ci, cell in enumerate(tr['tableCells']):
            ...
            if kind == 'header' or (ci == 0 and kind in ('act', 'chapter')):
                ...
                if kind in ('act', 'chapter'):
                    # заголовок Google Docs → строка попадает в оглавление документа
                    reqs.append({'updateParagraphStyle': {
