...
def main():
    ...
    # без обёрток-ссылок клик по картинке ничего не открывает — поправить подсказки
    h = h.replace("клик по картинке — крупно", "картинки встроены в файл")
    h = h.replace("Клик по картинке — PNG в 4K.", "4K-файлы экранов — в проекте: 00_Setup/05_Review/mockups.")
