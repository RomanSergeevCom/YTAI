...
| `ch_notes`? | `{ "NN": "…" }` | что с заставкой этой главы в кате (есть ли номер, где стоит, читается ли) — вкладка «Главы» |
| `ch_state`? | `{ "NN": ["что сейчас", "как надо"] }` | две колонки строки главы во вкладке «Главы» |
| `ch_extra`? | `[["tc", "…"], …]` | места, похожие на заставку главы, но главой не являющиеся (отдельные строки вкладки) |
| `ch_plate_variant`? | `"a" \| "b" \| "c"` | дизайн заставки главы (стадия J `make_infographics_v6`): `a` полотно · `b` шторка · `c` нижняя треть; по умолчанию `a` |
| `ch_demo_secs`? | `[sec, …]` | секунды БЕЗ экранного текста — подложка для демо `ch_demo_a|b|c` |
...
**Метка блока «Как надо» сменилась 22.09.2026: `✅ СДЕЛАТЬ` → `▶ СДЕЛАТЬ`** (Роман: «✅ — ощущение, что это знак
сделано»). Прежняя метка живёт ключом `core.lbl_do_legacy` и **не печатается**: её знают только разборщики текста
прошлых кругов (`pravki_prev_*.json` собраны с ней) — `pravki_lib.DO_LABELS` и `feedback_model.LABELS`. Ставить знак
литералом в коде нельзя ни в поверхностях, ни в проверках: только `T('core.lbl_do')` / `i18n.TL(lang, 'core.lbl_do')`.
