...
## Библиотека видов экранов канала (вне конвейера)

Каждый вид экрана должен быть виден **вне своей ТЗ** — иначе плашка живёт одним макетом внутри
ТЗ-42/43 и больше нигде (Роман 22.09.2026: «я не вижу плашки спикера во второй вкладке»).
Три файла, запускаются руками, все — локально и без токенов:

| что | файл | выход |
|---|---|---|
| текст про виды (когда применять, когда НЕ, где в фильме) | `shared/screen_kinds_ytuvi.py` | — редакторский источник, один на все поверхности |
| замер: буквы блоков, имена файлов, канон, примеры | `stages/screens_library.py [--check]` | `YTs/{CH}/screen_kinds.json` |
| свежесть картинок в публичной папке кадров | `stages/refresh_shots.py [--from-model] [--apply]` | перезалитые `shots_remote` + `shots_ids.json` |
| вкладка дока «Виды экранов · {ver}» (ОДНА таблица) | `stages/doc_tab_screens_v1.py [--dry-run] [--dump-requests F]` | вкладка |
| страница канала | репо сайта: `scripts/site/gen_ytuvi_screens.py [--scan\|--check\|--dry-run]` | `/ytuvi/screens/` |
| где в кадре ведущая (для раскладок, делящих кадр) | `stages/face_pos.py` — питон из `.venv_vlm` | `work/{cut}/face_pos.json` |
| «вот так и вот так, выбери» — страница сравнения макетов | `stages/compare_page.py --pair ФАЙЛ:ПОДПИСЬ …` | самодостаточный HTML |

## Карта выпуска и предложения экранов (вне конвейера)

Где в кате экрана нет — кто его предложит. Три шага руками, все локально и без токенов;
контракты — `docs/contracts_structure_screens.md`.

| что | файл | выход |
|---|---|---|
| места без экрана + дословная речь вокруг каждого | `stages/screens_spots.py` | `work/{cut}/screens_spots.json` |
| сами предложения — **пишет человек**, как `picks.json` у `kb_visuals` | — | `work/{cut}/screens_proposal.json` |
| гейт предложений: вид, поля, дословность, таймкод, чувствительное | `stages/screens_proposal.py check [--fix-quotes]` · `kinds` | exit 2 со списком |
| карта выпуска майндкартой (в одну сторону) | `stages/structure_xmind.py [--dump-titles F]` | `{CODE}_{cut}_structure.xmind` |

Предложения доезжают до двух поверхностей: ветки карты XMind и колонки «Как надо» во вкладке
«Главы» (`doc_tab_chapters_v1.py`). В `card.sub` они **не пишутся**: `sub_no_screen` и `sub_img` —
порядковые номера, новая подглава сдвинула бы их все.

⚠️ Картинку карты (`mockups/info_structure_map.png`) собирает блок H `make_infographics_v6.py`.
Вкладка сверяет её mtime с карточкой и `structure.json` и **не вставляет устаревшую**: иначе под
4K-листом идёт таблица, которая ему противоречит.

`screens_library.py --check` — контракт трёх источников: блок есть в коде (`want('X')`) → он описан
в таблице видов докстринга `make_infographics_v6.py` → у него есть файлы в `mockups/` → у него есть
карточка текста, и её «когда НЕ» ведёт на существующий вид. Расхождение = exit 2 со списком.

⚠️ **Шов деления кадра (`style.split_x`) не двигают под композицию кадра.** Раскладки, которые
делят кадр (шторка заставки, панель перечислений), держат шов числом канона — 1920 из 3840, ровно
половина. Ведущую в чистую половину приводит СДВИГ САМОГО КАДРА: `face_pos.py` меряет лица, рендер
сдвигает кадр вправо и ограничивает сдвиг шириной плиты (пустая полоса остаётся под ней).
Роман 22.09.2026: «вместо того, чтобы спикера подвинуть, экран подвинул?»

⚠️ **`ensure_shots` НЕ обновляет перезалитое**: он грузит только имена, которых нет в `shots_ids.json`.
Перерисованная под тем же именем плашка остаётся на Drive старой, и док показывает вчерашний дизайн
(замер 22.09.2026 на YTUVI01: 47 из 291 картинки отставали, включая `lower_tz42/43` — ту самую плашку
спикера — и все `fix_tz*`). Свежесть даёт `refresh_shots.py --apply`; `--from-model` дозаливает ещё и
примеры видов, которых нет в pravki (их `ensure_shots` не видит в принципе).
