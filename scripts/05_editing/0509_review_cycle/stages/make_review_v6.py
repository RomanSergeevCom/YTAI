...
PROJECT = Path(P.PROJECT_DIR)                # корень проекта — из карточки, без дефолта чужого фильма
REVIEW_DIR = P.REVIEW_DIR                    # папка карточки = {project}/00_Setup/05_Review
RENDER = Path(P.RENDER) if P.RENDER else PROJECT / '03_Exports'
MOCK = P.MOCK
REFS = REVIEW_DIR / 'refs'
FOOT = Path(P.resolve(P.get('footage_dir'))) if P.get('footage_dir') else PROJECT / '01_Source'
...
_MCOL = P.profile('ch_marker_colors') or ['Green', 'Cyan', 'Orange', 'Magenta', 'Blue', 'Yellow']
