...
PROG_DEMOS = [('z', 'ТЁМНАЯ СТРАНИЦА — ПРИНЯТ',
               'непрозрачная тёплая темнота: свежий пункт слоновой костью (15,7 : 1), пройденные '
               'приглушены до 45 %, рубин канала держит черту, тире и цифру счётчика'),
              ('h', 'СВЕТЛАЯ СТРАНИЦА — для примера',
               'та же вёрстка на странице слоновой кости, собственном приёме фильма: свежий пункт '
...
for i, (sec, no) in enumerate(CHAP):
    ...
    if pg:                                    # у главы есть панель перечисления — говорим об этом
        body += f'\n▸ панель «{pg.get("title", "")}» · {len(pg.get("items") or [])} пунктов'
    ...
    for n_, (s, t, j) in enumerate(mine):
        ...
        now = '⚠️ титра подтемы в кате нет' if s in NO_SCREEN else 'титр подтемы есть'
        do = 'подтему несёт панель перечисления' if pan else ''
        rows.append({'no': '', 'img': SUB_IMG.get(f'{j:02d}', ''), 'body': f'▸ {t}', 'ch': False,
                     'tc': f'{tmm(s)}–{tmm(s_end)}', 'now': now, 'do': do})
    if not mine:
        rows.append({'no': '', 'img': '', 'body': '▸ подглав в кате нет', 'ch': False,
                     'tc': '', 'now': '', 'do': ''})
...
head = [(1, f'{P.CODE} · {TAB_TITLE} — все главы фильма и дизайн заставок', {'bold': True}),
        ...
        (0, 'Та же карта одним кадром (4K, открывать в полный экран): ' + (
            f'https://drive.google.com/file/d/{shots["info_structure_map.png"]}/view'
            if shots.get('info_structure_map.png') else '— карта ещё не залита'), {}),
        ] + map_lines + [
        (0, '', {}),
