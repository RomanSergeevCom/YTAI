...
- Creates **`{project}_2_Assembly`** sequence:
  - **V1 only** — USE=TRUE segments in block order, each pre-trimmed to tc_in/tc_out
  ...
  - All markers are **Chapter type** (not Event or Comment)
...
### Stage 4: Captions (Assembly + Review)

After creating `edit_brief.json`, generate captions SRT for both timelines:
...
This reads your edit brief + per-clip transcript JSONs (word-level timing from 020101_transcribe) and generates:
- **`{project}_2_Assembly_captions.srt`** — for Assembly timeline
- **`{project}_3_Review_captions.srt`** — for Review timeline

The UXP plugin automatically imports both SRT files into 02_Transcripts during Build Assembly / Build Review. The editor then drags them onto the Caption track.
